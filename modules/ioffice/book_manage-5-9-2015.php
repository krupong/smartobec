	...
	switch ($_GET["action"]) {
		...
		case 'comment':
			if($_POST["bookid"]) {
				...
				//echo $bookstatusname;
				$message = "บันทึกเสนอเลขที่ ".$department_bookid." เรื่อง".$bookheader." [".$comment_personname."] ".$bookstatusname." : ".$commentdetail;
				//echo $message;
				smartpush("$post_personid",$message,"บันทึกเสนอสั่งการ");
			}
			break;
