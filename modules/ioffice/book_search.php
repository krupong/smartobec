...
?>
  ...
  <div class="box box-primary">
    <div class="box-header with-border">
      <p><h4 class="box-title">ค้นหา (รายการบันทึกข้อความทั้งหมด)</h4></p>
      <div class="row">
        <div class="col-md-12 text-right">
          <form class="form-inline" action="#" enctype="multipart/form-data" method="POST" >
            ...
            <a href="?option=ioffice&task=book_manage&action=search_clearsearchtext" class="btn btn-default" data-toggle="tooltip" data-placement="top" title="แสดงทั้งหมด"><span class="glyphicon glyphicon-list"></span></a>
          </form>
        </div>
      </div>
    </div>
    <div class="panel-body">
      ...
      // Check User
            ...
            // สารบัญสำนัก
            $sqlbookregister_permission = "SELECT
                                        bookregister_permission.id,
                                        bookregister_permission.person_id,
                                        bookregister_permission.p1,
                                        bookregister_permission.department,
                                        bookregister_permission.sub_department
                                        FROM
                                        bookregister_permission
                                        WHERE
                                        bookregister_permission.person_id = '".$_SESSION["login_user_id"]."' and
                                        bookregister_permission.p1=1 and bookregister_permission.saraban_status=2";
            $resultregister_permission = mysqli_query($connect,$sqlbookregister_permission);
            $num = mysqli_num_rows($resultregister_permission);
            if($num>0){
              $sqlpass = "post_departmentid = $user_department and ";
            }else{
              // สารบัญกลุ่ม
              $sqlbookregister_permission = "SELECT
                                          ...
                                          WHERE
                                          bookregister_permission.person_id = '".$_SESSION["login_user_id"]."' and
                                          bookregister_permission.p1=1 and bookregister_permission.saraban_status=3";
              $resultregister_permission = mysqli_query($connect,$sqlbookregister_permission);
              $num = mysqli_num_rows($resultregister_permission);
              if($num>0){
                $sqlpass = "post_subdepartmentid = $user_subdepartment and ";
            ...
            $sql = " SELECT 
                ...
                WHERE ".$sqlbookstatus.$sqldepartment." ".$sqlpass." ((bookid = '$searchtext') or (department_bookid like '%$searchtext%') or (bookheader like '%$searchtext%') or (pm1.name like '%$searchtext%'))
                ORDER BY ioffice_book.bookid DESC";
            ...
            <ul class="pagination">
              <?php 
                for($x=1;$x<=$page;$x++){
                  ...
                  echo "<li $active><a href='?option=ioffice&task=book_search&page=$x'>$x <span class='sr-only'></span></a></li>"; 
                }
              ?>
            </ul>
          </nav>
              ...
              while ($row = $result->fetch_assoc()) {
                ...
                ?>
                  <tr <?php echo $tr_class; ?>>
                    ...
                    <td>
                      <!-- Modal for Read -->
                      <button type="button" class="btn btn-info" data-toggle="modal" data-target="#myModal<?php echo $row['bookid']; ?>">เปิด</button>
                    ...
                    <td>
                      <?php
                        $sqlbookregister = "SELECT * FROM bookregister_send WHERE comment like '%".$row['department_bookid']."%'";
