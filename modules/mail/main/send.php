...
<div class="box box-success">
	<div class="box-header">
		...
		<h3 class="box-title">จดหมายที่ส่งแล้ว</h3>
	</div>
	<div class="box-body">
	<!-- ส่วนแสดงผล -->
<?php
...
<div class="box box-success">
	<div class="box-header">
        <i class="fa fa-sign-in"></i>
		<h3 class="box-title">เขียนจดหมาย</h3>
	</div>
	<div class="box-body">
	<!-- ส่วนแสดงผล -->
<?php
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B></Font>";
echo "</Cener>";
...
echo "<td width='514' colspan='3' align='left'>&nbsp;<input type='text' name='subject' size='76'  style='background-color: #FFFFCC'></td>";
echo "</tr>";

echo "<tr>";
echo "<td class='col-sm' align='right' height='47'><span lang='th'><font size='2' color='#0000FF'>ข้อความ&nbsp;</font></span></td>";
echo "<td height='47' width='514' colspan='3'  align='left'>&nbsp;<textarea rows='10' name='detail' cols='55'  style='background-color: #FFFFCC' ></textarea></td>";
echo "</tr>";
...
echo "<tr>";
echo "<td align='center' colspan='4'><BR><INPUT TYPE='button' class='btn btn-primary' name='smb' value='ตกลง' onclick='goto_url(1)'>&nbsp;&nbsp;<input type='reset' class='btn btn-default' value='ยกเลิก' name='reset'></td>";
echo "</tr>";
echo "</Table>";
echo "</form>";
?>
...
$sql="select * from mail_main left join  mail_sendto_answer  on  mail_main.ref_id=mail_sendto_answer.ref_id where mail_main.sender='$user' and  mail_sendto_answer.send_to='$person_id' order by ms_id desc ";
}
else{
...
echo "<table width='100%' align='center'><tr><Td  align='left'><INPUT TYPE='button' class='btn btn-primary' name='smb' value='เขียนจดหมาย' onclick='location.href=\"?option=mail&task=main/send&index=1\"'></Td><td align='right'>";
...
echo "&nbsp;<INPUT TYPE='button' name='smb' class='btn btn-info'  value='ค้นหา' onclick='goto_display(1)'>";
echo "&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;";

echo "ค้นหาด้วยชื่อผู้รับ&nbsp;</td>";
...
//echo  "<option  value = ''>เลือกบุคลากร</option>" ;
    $sql = "select  * from person_main where status=0 order by CONVERT(name USING tis620),CONVERT(surname USING tis620) ,position_code";
...
echo "&nbsp;&nbsp;<INPUT TYPE='button' name='smb' class='btn btn-warning' value='เลือก' onclick='goto_display(2)'>";   
echo "</td></tr></table>";
...
if($index==8 and $name_search!=""){
$sql="select * from mail_main where sender='$user'  and  subject like '%$name_search%' order by ms_id desc limit $start,$pagelen";
}
else if($index==8 and $person_id!=""){
$sql="select * from mail_main left join  mail_sendto_answer  on  mail_main.ref_id=mail_sendto_answer.ref_id where mail_main.sender='$user' and mail_sendto_answer.send_to='$person_id' order by ms_id desc limit $start,$pagelen";
}else{
$sql="select * from mail_main where sender='$user' order by ms_id desc limit $start,$pagelen";
}
$dbquery = mysqli_query($connect,$sql);
echo  "<table width='85%' border='1' align=center style='border-collapse: collapse' class='table table-striped table-bordered table-hover'>";
echo "<Tr bgcolor='#99ccff' class='info'  align='center'><Td class='col-sm'>เลขที่</Td><Td class='col-sm'>วันที่ส่ง</Td><Td class='col-sm'>เรื่อง</Td><Td class='col-sm'>ส่งถึง</Td><Td class='col-sm'>ลบ</Td></Tr>";
...
echo thai_date_4($rec_date);
			...
			if($num_file>=1){
			echo"<td align='top' align='left'><a href=?option=mail&task=main/send&index=7&id=$id&page=$page&name_search=$name_search&person_id=$person_id>$subject</a><img src=images/file1.gif></td>";
			}
			else{
			echo "<td><a href=?option=mail&task=main/send&index=7&id=$id&page=$page&name_search=$name_search&person_id=$person_id>$subject</a></td>";
			}
