...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ตำแหน่งบุคลากรในสำนักงานเขตพื้นที่การศึกษา</strong></font></td></tr>";
echo "</table>";
}
//ส่วนเพิ่มข้อมูล
if($index==1){
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=person&task=khet_position&index=3&id=$_GET[id]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=person&task=khet_position\"'";
echo "</td></tr></table>";
}
//ส่วนลบข้อมูล
if($index==3){
$sql = "delete from person_khet_position where id=$_GET[id]";
$dbquery = mysqli_query($connect,$sql);
}

//ส่วนเพิ่มข้อมูล
if($index==4){
$sql = "insert into person_khet_position (position_code, position_name) values ( '$_POST[position_code]','$_POST[position_name]')";
$dbquery = mysqli_query($connect,$sql);
}
...
echo "</Cener>";
echo "<Br>";
$sql = "select * from  person_khet_position where id='$_GET[id]'";
$dbquery = mysqli_query($connect,$sql);
...
//ส่วนปรับปรุงข้อมูล
if ($index==6){
$sql = "update person_khet_position set position_code='$_POST[position_code]',position_name='$_POST[position_name]' where id='$_POST[id]'";
$dbquery = mysqli_query($connect,$sql);
}
...
echo  "<table width=50% border=0 align=center>";
echo "<Tr><Td colspan='6' align='left'><INPUT TYPE='button' name='smb' value='เพิ่มข้อมูล' onclick='location.href=\"?option=person&task=khet_position&index=1\"'></Td></Tr>";
...
While ($result = mysqli_fetch_array($dbquery))
	{
		...
		echo "<Tr  bgcolor=$color align=center class=style1><Td>$N</Td> <Td>$position_code</Td> <Td align=left>$position_name</Td><Td><div align=center><a href=?option=person&task=khet_position&index=2&id=$id><img src=images/drop.png border='0' alt='ลบ'></a></div></Td><Td><a href=?option=person&task=khet_position&index=5&id=$id><img src=images/edit.png border='0' alt='แก้ไข'></a></div></Td>
	</Tr>";
...
function goto_url(val){
	if(val==0){
		callfrm("?option=person&task=khet_position");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.position_code.value == ""){
			...
			callfrm("?option=person&task=khet_position&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.position_code.value == ""){
			...
			callfrm("?option=person&task=khet_position&index=6");   //page ประมวลผล
		}
	}
}
</script>
