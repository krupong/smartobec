...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=person&task=person_khet&index=3&id=$_GET[id]&page=$_REQUEST[page]&khet_code=$_REQUEST[khet_code]\"'>
		&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=person&task=person_khet&page=$_REQUEST[page]&khet_code=$_REQUEST[khet_code]\"'";
echo "</td></tr></table>";
}
...
$url_link="option=person&task=person_khet&page_var1=$_REQUEST[khet_code]&index=$index&name_search=$_REQUEST[name_search]";
...
echo "<table width='95%' align='center'><tr><Td align='left'><INPUT TYPE='button' name='smb' value='เพิ่มข้อมูล' onclick='location.href=\"?option=person&task=person_khet&index=1\"'></Td><td align='right'>";
...
echo "<Td><div align='center'><a href=?option=person&task=person_khet&index=2&id=$id&page=$page&khet_code=$khet_code><img src=images/drop.png border='0' alt='ลบ'></a></div></Td>
		<Td><a href=?option=person&task=person_khet&index=5&id=$id&page=$page&khet_code=$khet_code><img src=images/edit.png border='0' alt='แก้ไข'></a></div></Td>
	</Tr>";
$M++;
$N++;
	}
...
function goto_url(val){
	if(val==0){
		callfrm("?option=person&task=person_khet");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.person_id.value == ""){
			...
			callfrm("?option=person&task=person_khet&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.person_id.value == ""){
			...
			callfrm("?option=person&task=person_khet&index=6");   //page ประมวลผล
		}
	}
}
...
function goto_display(val){
	if(val==1){
		callfrm("?option=person&task=person_khet"); 
		}
	else if(val==2){
		callfrm("?option=person&task=person_khet&index=8"); 
		}
}

function goto_delete_all(){
			callfrm("?option=person&task=person_khet&index=3.1"); 
}
