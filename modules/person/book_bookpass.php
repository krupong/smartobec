...
<div class="container">
  <div class="panel panel-default">
    <div class="panel-heading">
      <h3 class="panel-title">กำหนดรองเลขาฯ / ผู้ช่วยเลขาฯ ประจำสำนัก</h3>
    </div>
      ...
      <div class="row">
        <div class="col-md-12 text-right">
          <form class="form-inline" action="#" enctype="multipart/form-data" method="POST" >
            ...
            <a href="?option=person&task=book_manage&action=bookpass_clearsearchtext" class="btn btn-default"><span class="glyphicon glyphicon-list"></span> แสดงทั้งหมด</a>
          </form>
        </div>
      </div>
      <hr>
        ...
        <tbody>
          ...
          <?php 
            ...
            $sql = "  SELECT
                      ...
                      WHERE
                      (person_main.position_code = 2 or person_main.position_code = 3) and person_main.name like '%$searchtext%'
                      ORDER BY person_main.position_code,person_main.name";
                //echo $sql;
              ...
              while ($row = $result->fetch_assoc()) {
                ?>
                  <tr>
                    ...
                    <td>
                      <?php 
                      $sqlpasscount = "SELECT * FROM ioffice_bookpass ib LEFT JOIN system_department sd ON(ib.departmentid=sd.department) WHERE personid = '".$row['person_id']."'";
                      if($resultpasscount = mysqli_query($connect,$sqlpasscount)){
                        $dep="";
                        while ($rowpasscount = $resultpasscount->fetch_assoc()) {
                          if($dep!=""){ $dep.=" / "; }
                          $dep.=$rowpasscount["department_precis"];
                        }
                      }
                      $countpasscount = mysqli_num_rows($resultpasscount);
                      echo "<span class='badge progress-bar-success'>".$countpasscount."</span> ".$dep; 
                      ?>
                    </td>
                    <td>
                      <!-- Modal for Read -->
                      ...
                      <form enctype="multipart/form-data" class="form-horizontal" method="POST" action="?option=person&task=book_manage&action=bookpass">
