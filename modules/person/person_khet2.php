...
$areaid = $_SESSION['system_user_khet'];

if($total_permission_sch['p4']==1){	
exit();
}
...
function file_upload() {
	...
	//	$uploaddir = '../../upload/person/picture/';      //ที่เก็บไไฟล
...
echo "<Tr align='left'><Td width=30 height=40></Td><Td align='right'>เลขประจำตัวประชาชน&nbsp;</Td><Td><Input Type='Text' Name='person_id' Size='13' maxlenght='13' onkeydown='integerOnly()'></Td></Tr>";
echo "<Tr align='left'><Td width=30 height=40></Td><Td align='right'>คำนำหน้าชื่อ&nbsp;</Td><Td><Input Type='Text' Name='prename' Size='15'></Td></Tr>";
echo "<Tr align='left'><Td ></Td><Td align='right' height=40>ชื่อ&nbsp;</Td><Td><Input Type='Text' Name='name'  Size='40'></Td></Tr>";
echo "<Tr align='left'><Td ></Td><Td align='right' height=40>นามสกุล&nbsp;</Td><Td><Input Type='Text' Name='surname'  Size='40'></Td></Tr>";

echo "<Tr align='left'><Td ></Td><Td align='right' height=40>ตำแหน่ง&nbsp;</Td><Td>";
...
echo "<Tr align='left'><Td ></Td><Td align='right' height=40>สพท&nbsp;</Td><Td>";
echo "<Select  name='khet_code' size='1'>";
//echo  "<option  value = ''>เลือก</option>" ;

$sql = "select * from  system_khet where khet_code ='$areaid' order by khet_type,khet_code";
$dbquery = mysqli_query($connect,$sql);
...
echo "<Tr align='left'><Td ></Td><Td align='right' height=40>ลำดับบุคคลในตำแหน่ง&nbsp;</Td><Td><Input Type='Text' Name='person_order'  Size='4'>&nbsp;(หากประสงค์เรียงลำดับ)</Td></Tr>";

echo  "<tr align='left'>";
echo  "<Td ></Td><td align='right' height=40>ไฟล์รูปภาพ&nbsp;</td>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=person&task=person_khet2&index=3&id=$_GET[id]&page=$_REQUEST[page]&khet_code=$_REQUEST[khet_code]\"'>
		&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=person&task=person_khet2&page=$_REQUEST[page]&khet_code=$_REQUEST[khet_code]\"'";
echo "</td></tr></table>";
}

//ส่วนลบข้อมูล
if($index==3){
$sql = "delete from person_khet_main where id=$_GET[id]";
$dbquery = mysqli_query($connect,$sql);
}

if($index==3.1){
	foreach ($_POST as $person_id =>$person_value){
$sql = "delete from person_khet_main where person_id='$person_id'";
$dbquery = mysqli_query($connect,$sql);
	}
}
...
if($index==4){
$rec_date = date("Y-m-d");
$sql = "select * from person_khet_main where  person_id='$_POST[person_id]' ";
$dbquery = mysqli_query($connect,$sql);
...
$sql = "insert into person_khet_main (person_id,prename,name,surname,position_code,khet_code,pic,status,person_order,officer,rec_date) values ( '$_POST[person_id]','$_POST[prename]','$_POST[name]','$_POST[surname]','$_POST[position_code]','$_POST[khet_code]','$changed_name','0','$_POST[person_order]','$officer','$rec_date')";
$dbquery = mysqli_query($connect,$sql);
}
...
echo "<form Enctype = multipart/form-data id='frm1' name='frm1'>";
echo "<Center>";
echo "<Font color='#006666' Size=3><B>แก้ไขข้อมูล/ย้ายออก บุคลากร</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
$sql = "select * from  person_khet_main where id='$_GET[id]'";
$dbquery = mysqli_query($connect,$sql);
$result = mysqli_fetch_array($dbquery);
echo "<Table width='70%' Border='0'>";
echo "<Tr align='left'><Td width=30 height=40></Td><Td align='right'>เลขประจำตัวประชาชน&nbsp;</Td><Td><Input Type='Text' Name='person_id' Size='13' maxlenght='13' onkeydown='integerOnly()' value='$result[person_id]'></Td></Tr>";
echo "<Tr align='left'><Td width=30 height=40></Td><Td align='right'>คำนำหน้าชื่อ&nbsp;</Td><Td><Input Type='Text' Name='prename' Size='15' value='$result[prename]'></Td></Tr>";
echo "<Tr align='left'><Td height=40></Td><Td align='right'>ชื่อ&nbsp;</Td><Td><Input Type='Text' Name='name'  Size='40' value='$result[name]'></Td></Tr>";
echo "<Tr align='left'><Td height=40></Td><Td align='right'>นามสกุล&nbsp;</Td><Td><Input Type='Text' Name='surname'  Size='40' value='$result[surname]'></Td></Tr>";
echo "<Tr align='left'><Td height=40></Td><Td align='right'>ตำแหน่ง&nbsp;</Td><Td>";
echo "<Select  name='position_code' size='1'>";
echo  "<option  value = ''>เลือก</option>" ;
$sql = "select * from  person_khet_position order by position_code";
$dbquery = mysqli_query($connect,$sql);
...
$dbquery = mysqli_query($connect,$sql);
While ($result_khet = mysqli_fetch_array($dbquery)){
		if($result['khet_code']==$result_khet['khet_code']){
...
echo "<Tr align='left'><Td ></Td><Td align='right' height=40>ลำดับบุคคลในตำแหน่ง&nbsp;</Td><Td><Input Type='Text' Name='person_order'  Size='4' value='$result[person_order]'></Td></Tr>";
...
//ส่วนปรับปรุงข้อมูล
if ($index==6){
$sql = "select * from person_khet_main where person_id='$_POST[person_id]' and id!='$_POST[id]' ";
$dbquery = mysqli_query($connect,$sql);
if(mysqli_num_rows($dbquery)>=1){
echo "<br /><div align='center'>มีเลขประจำตัวประชาชนซ้ำกับรายการที่มีอยู่แล้ว ตรวจสอบอีกครั้ง</div>";
exit();  
		...
		if ($basename!=""){
		$changed_name = file_upload();
		$sql = "update person_khet_main set person_id='$_POST[person_id]', prename='$_POST[prename]', name='$_POST[name]', surname='$_POST[surname]', pic='$changed_name', position_code='$_POST[position_code]', khet_code='$_POST[khet]', person_order='$_POST[person_order]',officer='$officer' where id='$_POST[id]'";
		}
		else{
		$sql = "update person_khet_main set person_id='$_POST[person_id]', prename='$_POST[prename]', name='$_POST[name]', surname='$_POST[surname]', position_code='$_POST[position_code]', khet_code='$_POST[khet]', person_order='$_POST[person_order]',officer='$officer' where id='$_POST[id]'";
		}
$dbquery = mysqli_query($connect,$sql);
$index="";
}
...
$sql_page=  "select * from  person_khet_main  left join system_khet on person_khet_main.khet_code=system_khet.khet_code  where khet_code=$areaid and person_khet_main.name like '%$_REQUEST[name_search]%'  or person_khet_main.surname like '%$_REQUEST[name_search]%' or system_khet.khet_name like '%$_REQUEST[name_search]%' and person_khet_main.status='0'  ";
$_REQUEST['khet_code']="";
}
else if($_REQUEST['khet_code']==""){
$sql_page = "select id from person_khet_main where khet_code ='$areaid' and status='0' ";
}
else{
...
$url_link="option=person&task=person_khet2&page_var1=$_REQUEST[khet_code]&index=$index&name_search=$_REQUEST[name_search]";
...
echo "<table width='95%' align='center'><tr><Td align='left'><INPUT TYPE='button' name='smb' value='เพิ่มข้อมูล' onclick='location.href=\"?option=person&task=person_khet2&index=1\"'></Td><td align='right'>";
...
echo "<Td><div align='center'><a href=?option=person&task=person_khet2&index=2&id=$id&page=$page&khet_code=$khet_code><img src=images/drop.png border='0' alt='ลบ'></a></div></Td>
		<Td><a href=?option=person&task=person_khet2&index=5&id=$id&page=$page&khet_code=$khet_code><img src=images/edit.png border='0' alt='แก้ไข'></a></div></Td>
	</Tr>";
$M++;
$N++;
	}
...
function goto_url(val){
	if(val==0){
		callfrm("?option=person&task=person_khet2");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.person_id.value == ""){
			...
			callfrm("?option=person&task=person_khet2&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.person_id.value == ""){
			...
			callfrm("?option=person&task=person_khet2&index=6");   //page ประมวลผล
		}
	}
}
...
function goto_display(val){
	if(val==1){
		callfrm("?option=person&task=person_khet2"); 
		}
	else if(val==2){
		callfrm("?option=person&task=person_khet2&index=8"); 
		}
}

function goto_delete_all(){
			callfrm("?option=person&task=person_khet2&index=3.1"); 
}
