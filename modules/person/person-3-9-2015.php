    ...
    <div class="panel-body">
      <div class="row">
        <div class="col-md-2 text-left">
          <a href="?option=person&task=person_insert&page=<?php echo $_GET["page"]; ?>" class="btn btn-primary"><span class="glyphicon glyphicon-plus"></span>&nbsp;เพิ่มบุคลากร</a>
        </div>
        <div class="col-md-10 text-right">
          <form class="form-inline" action="#" enctype="multipart/form-data" method="POST" >
            ...
            <a href="?option=person&task=person_manage&action=select_clearsearchtext" class="btn btn-default"><span class="glyphicon glyphicon-list"></span> แสดงทั้งหมด</a>
          </form>
        </div>
      </div>
      <hr>
      ...
      $sql = "  SELECT 
                ...
                WHERE 
                  ((pm.person_id like '%$searchtext%') or (pm.name like '%$searchtext%') or (pm.surname like '%$searchtext%'))
                  $sqldepartment     
                ORDER BY 
            ...
            <ul class="pagination">
              <?php 
                for($x=1;$x<=$page;$x++){
                  ...
                  echo "<li $active><a href='?option=person&task=person&page=$x'>$x <span class='sr-only'></span></a></li>"; 
                }
              ?>
            </ul>
          </nav>
              ...
              while ($row = $result->fetch_assoc()) {
                ?>
                  <tr>
                    ...
                    <td><form action="?option=person&task=person_update&page=<?php echo $_GET["page"]; ?>" enctype="multipart/form-data" method="POST">
                        <button class="btn btn-warning" type="submit" name="update" id="update" value="<?php echo $row['person_id']; ?>"><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></button>
                        </form>
                    </td>      
                    <td><form action="?option=person&task=person_manage&page=<?php echo $_GET["page"]; ?>" enctype="multipart/form-data" method="POST">
                        <button class="btn btn-danger" type="submit" name="delete" id="delete" value="<?php echo $row['person_id']; ?>" data-toggle="confirmation"><span class='glyphicon glyphicon-trash' aria-hidden='true'></span></button>
                        </form>
                    </td>
                  </tr>
                <?php
