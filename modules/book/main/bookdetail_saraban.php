...
if($_POST['index']==1){
		if (isset($_POST['s_id'])) 
			{ 
				...
				mysqli_query($connect,"update bookregister_receive set department='$_POST[s_id]' where ref_id='$_POST[ref_id]' ") ;
			}
}
...
if ($ans_num>0) {
$day_now=date("Y-m-d H:i:s");
$sql_answer = mysqli_query($connect,"update book_sendto_answer set answer='1', answer_time='$day_now' where ref_id='$ref_id' and send_to='saraban'") ;
				...
				$sql = "insert into bookregister_receive(year, register_number, book_no, signdate, book_from, book_to, subject, register_date, ref_id, officer, book_link, secret) values ('$result_start[year]', '$register_number', '$book_no', '$row2[signdate]', '$sender', 'สำนักงานเขตพื้นที่การศึกษา', '$subject', '$day_now', '$ref_id', '$user', '$book_link', '$secret')";
...
if($row2['secret']==1){
				...
				?>
									<tr>
										<td align="left">&nbsp;<FONT SIZE="2"><?php echo $list;?>. </FONT><A HREF="../../../upload/book/upload_files/<?php echo $road.$file_name;?>" title="คลิกเพื่อเปิดไฟล์แนบลำดับที่ <?php echo $list;?>" target="_BLANK"><FONT SIZE="2" COLOR="#CC0099"><span style="text-decoration: none"><?php echo $file_des;?></span></FONT></A></td>
									</tr>
				<?php
}
//endxx				
...
echo "<tr>";
$result1=mysqli_query($connect,"SELECT * FROM system_department") ;
...
$num_select = mysqli_num_rows ($result_select) ;
			if ($num_select==0) {
			   ?>
				  <TD  width="25%" align="left">&nbsp;&nbsp;&nbsp;<input type="radio" name="s_id" value="<?php echo $department;?>"><FONT SIZE="2"><?php echo $department_name;?></FONT></TD>
  ...
  <div align="center">
	<table border="0" width="400"  style="border-collapse: collapse">
		<form method="POST" action="" name="form2" >
			<tr>
				<td>&nbsp;<b><font size="2" color="#003399">รายชื่อสำนักที่ส่งหนังสือให้แล้ว
