...
if($_POST['index']==1){
		//ส่งคืน สถานะ 4 ส่งให้เจ้าหน้าที่ในกลุ่มสถานะ 2
		...
		if (isset($_POST['s_id'])) 
			{ 
				...
				mysqli_query($connect,"INSERT INTO book_sendto_answer (send_to, ref_id, school, status, forward_from, rec_forward_date) Values('$_POST[s_id]','$_POST[ref_id]', '$result_permission[p2]','$forward_status','$_POST[forward_name]','$day_now') ") ;
...
/////////////
$department = $result_permission['p2'];
$sub_department  = $_SESSION['system_user_subdepartment'];  //รหัสกลุ่ม
//ส่วนการบันทึกทะเบียนรับส่งหนังสือราชการ
$sql_answer = mysqli_query($connect,
//"SELECT id FROM  book_sendto_answer WHERE send_level='2' and ref_id ='$ref_id' and send_to='$result_permission[p2]' and answer is null ") ;
"SELECT id FROM  book_sendto_answer WHERE ref_id ='$ref_id' and send_to='$result_permission[p2]' and answer is null ") ;
  $ans_num = mysqli_num_rows ($sql_answer) ;

if ($ans_num>0) {
$day_now=date("Y-m-d H:i:s");

				//หาว่ามีการลงทะเบียนสพฐ.แล้วหรือยัง
//				$sql_chk = mysqli_query($connect, "SELECT ref_id FROM  bookregister_receive WHERE ref_id ='$ref_id'") ;
				$sql_chk = mysqli_query($connect, "SELECT ref_id,department FROM  bookregister_receive WHERE ref_id ='$ref_id' and department='$department'") ;
  			 	$result_chk = mysqli_num_rows ($sql_chk) ;	 		
			 	//echo $result_chk;
				...
				//เลขสำนัก
				$sql_check_bookregister_de = mysqli_query($connect,"SELECT * FROM  bookregister_year WHERE year_active='1' and  start_receive_num!='0' and department = '$department' ") ;
				if($result_start_de=mysqli_fetch_array($sql_check_bookregister_de)){
				//เลขทะเบียน
	...
	//echo $register_number_de;
				...
				if($result_chk >=1){
				//$sql = "update bookregister_receive set register_number_de=$register_number_de, department = $department where ref_id='$ref_id' ";
				$sql = "update bookregister_receive set register_number_de=$register_number_de, department = $department where ref_id='$ref_id' and department='$department'";
				$dbquery = mysqli_query($connect,$sql);
				//echo $sql;
				}
				else{
				//ถ้ายังไม่ลงรับใช้บันทึกใหม่
				$sql = "insert into bookregister_receive(year, register_number, register_number_de, book_no, signdate, book_from, book_to, subject, department, record_type, register_date, ref_id, officer, level) values ('$result_start_de[year]', '$register_number', '$register_number_de', '$book_no', '$row2[signdate]', '$sender', '1', '$subject', '$department', '1','$day_now', '$ref_id', '$user', '$book_link')";
				$dbquery = mysqli_query($connect,$sql);
				}
				}
				}
...
//select เลขทะเบียนหนังสือรับ สำนัก
				$query_register_num=mysqli_query($connect,"select  register_number_de from bookregister_receive where ref_id='$ref_id' and department='$department'");
...
$day_now=date("Y-m-d H:i:s");
$sql_answer = mysqli_query($connect,
"update book_sendto_answer set answer='1', 
		answer_time='$day_now'
		where ref_id='$ref_id' and send_to='$result_permission[p2]'") ;
} //end if ans_num
...
if ($level==1) {
	...
	<table border="0" width="480" id="table1" style="border-collapse: collapse; border: 1px dotted #FF00FF; ; padding-left:4px; padding-right:4px; padding-top:1px; padding-bottom:1px" cellpadding="2" >
		...
		<tr>
			...
			<font size="2">&nbsp;เลขทะเบียนหนังสือรับ สำนัก: </font> <FONT SIZE="2" COLOR="#CC3300"><?php echo $result_register_num['register_number_de']; ?></font></td>
		</tr>
		<tr>
			<td width="449" align="left" colspan="2" style="border: 1px dotted #808000">
...
<table border="0" width="100%" style="border-collapse: collapse">
  ...
  <form method="POST" action="bookdetail_department.php" name="form1" onSubmit="return checkform();">
  <TABLE border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" width=95%>

<?php
echo "<tr>";
$result1=mysqli_query($connect,
"SELECT * FROM   system_subdepartment where department='$result_permission[p2]'") ;
$num1 = mysqli_num_rows ($result1) ;

$list1=1;
while ($r1=mysqli_fetch_array($result1)) {
	$sub_department = $r1['sub_department'] ;
	$sub_department_name = $r1['sub_department_name'] ;
	
$result_select=mysqli_query($connect,
"SELECT * FROM  book_sendto_answer WHERE send_to='$sub_department_name' and ref_id='$ref_id' order by id") ;
$num_select = mysqli_num_rows ($result_select) ;
			if ($num_select==0) {
			   ?>
				  <TD  width="25%" align="left">&nbsp;&nbsp;&nbsp;<input type="radio" name="s_id" value="<?php echo $sub_department;?>"><FONT SIZE="2"><?php echo $sub_department_name;?></FONT></TD>
...
<?php
$result2=mysqli_query($connect,"SELECT * FROM book_sendto_answer WHERE ref_id='$ref_id' and (status='2' or status='6')");
...
while ($r2=mysqli_fetch_array($result2)) {
	$send_to = $r2['send_to'] ;
?>&nbsp;<FONT SIZE="2">&nbsp;<FONT SIZE="2"><A HREF="bookdetail_department.php?index=2&sd_index=<?php echo $r2['id'];?>&b_id=<?php echo $_REQUEST['b_id'];?> "><IMG SRC="../../../images/b_drop.png" WIDTH="16" HEIGHT="16" BORDER="0" ALT="ลบออก"></A>&nbsp; <?php echo $list2;?>. <?php echo $office_name_ar[$send_to]; ?>&nbsp; 
