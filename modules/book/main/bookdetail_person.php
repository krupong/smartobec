...
if(isset($_POST['return'])){
		if (isset($_POST['s_id'])) 
			{ 
				...
				mysqli_query($connect,"INSERT INTO book_sendto_answer (send_to, ref_id, school, status, forward_from, rec_forward_date) Values('$_POST[s_id]','$_POST[ref_id]', '$_POST[office_from]','4','$_POST[forward_name]','$day_now') ") ;
			}
}
...
//select เลขทะเบียนหนังสือรับ
				$query_register_num=mysqli_query($connect,"select  register_number_g from bookregister_receive where ref_id='$ref_id' and sub_department='$sub_department'");
...
if ($ans_num == 0) {
$day_now=date("Y-m-d H:i:s");
$sql_answer = mysqli_query($connect,"update book_sendto_answer set answer='1', answer_time='$day_now'	where ref_id='$ref_id' and send_to='$user'") ;
}
...
if ($level==1) {
	...
	<table border="0" width="480" id="table1" style="border-collapse: collapse; border: 1px dotted #FF00FF; ; padding-left:4px; padding-right:4px; padding-top:1px; padding-bottom:1px" cellpadding="2" >
		...
		<tr>
			...
			<font size="2">&nbsp;เลขทะเบียนหนังสือรับ กลุ่ม : </font> <FONT SIZE="2" COLOR="#CC3300"><?php echo $result_register_num['register_number_g']; ?></font></td>
		</tr>
		<tr>
			<td width="449" align="left" colspan="2" style="border: 1px dotted #808000">
...
<table border="0" width="100%" style="border-collapse: collapse">
  ...
  <form method="POST" action="bookdetail_person.php" name="form1" onSubmit="return checkform();">
...
<?php
$result2=mysqli_query($connect,"SELECT * FROM book_sendto_answer WHERE ref_id='$ref_id' and status='4' ");
...
while ($r2=mysqli_fetch_array($result2)) {
	$send_to = $r2['send_to'] ;
?>&nbsp;<FONT SIZE="2" COLOR="">&nbsp;<FONT SIZE="2" COLOR=""><A HREF="bookdetail_person.php?index=2&sd_index=<?php echo $r2['id'];?>&b_id=<?php echo $_REQUEST['b_id'];?> "><IMG SRC="../../../images/b_drop.png" WIDTH="16" HEIGHT="16" BORDER="0" ALT="ลบออก"></A>&nbsp; <?php echo $list2;?>. <?php echo $office_name_ar[$send_to]; ?>&nbsp; 
