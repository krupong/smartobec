...
if($_POST['index']==1){
		if (isset($_POST['s_id'])) 
			{ 
				...
				mysqli_query($connect,"INSERT INTO book_sendto_answer (send_to, school, ref_id, status, forward_from, rec_forward_date) Values('$_POST[s_id]', '$_SESSION[user_school]', '$_POST[ref_id]', '5', '$_POST[forward_name]','$day_now') ") ;
				
				// ส่วนบันทึกผู้ปฏิบัติในทะเบียนรับหนังสือราชการ
				$sql_operation = mysqli_query($connect,"SELECT * FROM  person_sch_main WHERE person_id='$_POST[s_id]'");
				...
				mysqli_query($connect,"update bookregister_receive_sch set operation='$operation_name'
				where ref_id='$_POST[ref_id]' and school_code='$_SESSION[user_school]'") ;
			}
}
...
// saraban answer
$sql_answer = mysqli_query($connect,"SELECT id FROM  book_sendto_answer WHERE ref_id ='$ref_id' and send_to='$_SESSION[user_school]' and answer is null") ;
...
$sql_answer = mysqli_query($connect,"update book_sendto_answer set answer='1', 
		answer_time='$day_now'
		where ref_id='$ref_id' and send_to='$_SESSION[user_school]'") ;
		
				//ส่วนการบันทึกทะเบียนรับส่งหนังสือราชการ
				$sql_check_bookregister = mysqli_query($connect,"SELECT * FROM  bookregister_year WHERE  school_code='$_SESSION[user_school]' and year_active='1' and  start_receive_num!='0' ") ;
				if($result_start=mysqli_fetch_array($sql_check_bookregister)){
				//เลขทะเบียน
				$sql_number="select  max(register_number) as number_max from bookregister_receive_sch where year='$result_start[year]' and school_code='$_SESSION[user_school]' ";
...
<table border="0" width="100%" style="border-collapse: collapse">
		<tr>
			<td bgcolor="#003399"><font face="Tahoma"><font size="2">&nbsp;</font><span lang="th"><font size="2" color="#FFFFFF"><B>กรุณากำหนดผู้รับหนังสือไปดำเนินการ</B></font></span></font> </td>
		</tr>
		</table>

  <form method="POST" action="bookdetail_school_saraban.php" name="form1" onSubmit="return checkform();">
  <TABLE border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" width=95%>

<?php
if($result_permission){
echo "<tr>";
$result1=mysqli_query($connect,"SELECT * FROM person_sch_main where school_code='$_SESSION[user_school]' and status='0' order by name") ;
...
$result_select=mysqli_query($connect,"SELECT * FROM  book_sendto_answer WHERE send_to='$person_id' and ref_id='$ref_id' and school='$_SESSION[user_school]' and status='5' ") ;
...
//บุคลากรในตารางขยาย
echo "<tr>";
$result1=mysqli_query($connect, "select * from  person_sch_other left join person_sch_main on person_sch_other.person_id=person_sch_main.person_id where person_sch_main.status='0' and person_sch_other.status='0'  and person_sch_other.school_code='$_SESSION[user_school]' ") ;
$num1 = mysqli_num_rows ($result1) ;
...
<BR><input name="ref_id" type="hidden"  value="<?php echo $ref_id;?>"><input name="forward_name" type="hidden"  value="<?php echo $user;?>"><input name="index" type="hidden"  value="1"><input name="b_id" type="hidden"  value="<?php echo $_REQUEST['b_id'];?>">
	 <CENTER><input type="submit" value="  เลือก  " name="submit">
<HR>	</form>


<!--แสดงผู้ที่เคยส่งหนังสือให้ -->
<?php
$result2=mysqli_query($connect,"SELECT book_sendto_answer.id, book_sendto_answer.rec_forward_date, person_sch_main.prename, person_sch_main.name, person_sch_main.surname FROM book_sendto_answer, person_sch_main where book_sendto_answer.send_to=person_sch_main.person_id and book_sendto_answer.ref_id='$ref_id' and school='$_SESSION[user_school]' and  book_sendto_answer.status='5' ") ;
...
?>&nbsp;<FONT SIZE="2" COLOR=""><A HREF="bookdetail_school_saraban.php?index=2&sd_index=<?php echo $r2['id'];?>&b_id=<?php echo $_REQUEST['b_id'];?> "><IMG SRC="../../../images/b_drop.png" WIDTH="16" HEIGHT="16" BORDER="0" ALT="ลบออก"></A>&nbsp; <?php echo $list2;?>. <?php echo $r2['prename'].$r2['name']." ".$r2['surname'] ;?>&nbsp; <?php
