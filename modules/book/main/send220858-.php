...
if(($_SESSION['login_group']>1) or ($_SESSION['login_status']>10) and ($_SESSION['login_status']<=14)){
...
echo "<td  colspan='3' align='left'>";
	$sql_school= "SELECT * FROM  system_khet where  khet_code = '$khet_code'";
...
echo "<tr>";
echo "<td align='right'><span lang='th'>เลขที่หนังสือ&nbsp;</span></td><td>&nbsp;<input type='text' name='book_no' size='10' value='ที่'  style='background-color: #99ccff' required>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>";
...
if($_SESSION['login_group']==1){
			if($_POST['sendto']=='all') { 
			$sql_sendto = "select khet_code from system_khet  where  khet_type='1' or  khet_type='2 order by khet_code";
...
if(($_SESSION['login_group']>1) or ($_SESSION['login_status']>10) and ($_SESSION['login_status']<=14)){		
			if($_POST['sendto']=='all') { 
			$sql_sendto = "select khet_code from system_khet where khet_code != '$_SESSION[user_khet]' and khet_type='1' or  khet_type='2'  order by khet_code";
			$dbquery_sendto = mysqli_query($connect,$sql_sendto);
					While ($result_sendto = mysqli_fetch_array($dbquery_sendto)){
...
//ส่วนของการแยกหน้า
	...
	$num_rows = mysqli_num_rows($dbquery);
	...
	else if(($_SESSION['login_group']>1) or ($_SESSION['login_status']>10) and ($_SESSION['login_status']<=15)){
			if($_REQUEST['search_index']==1){
			      if($khet_code!=""){
			...
			$sql="select * from book_main where book_type='2' and office='$khet_code' order by ms_id desc";
			}
			}
			else{
			$sql="select * from book_main where book_type='2'and office='$khet_code' order by ms_id desc";
