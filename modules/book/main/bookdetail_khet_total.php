...
$sql_sender = mysqli_query($connect,"SELECT * FROM  system_khet WHERE  school_code='$row2[office]' ") ;
$row_sender= mysqli_fetch_array($sql_sender) ;
$sender=$row_sender[school_name];
		//หาชื่อผู้ส่ง	
		$sql_name = mysqli_query($connect,"SELECT * FROM  person_khet_main WHERE person_id='$row2[sender]'");
		$row_name= mysqli_fetch_array($sql_name) ;
}
...
$sql_person = mysqli_query($connect,"SELECT * FROM  person_khet_main") ;
...
$sql_name = "select * from book_sendto_answer where ref_id='$ref_id' and send_to='$khet_code' order by id ";
