...
echo "<td align='center' colspan='4'><BR><INPUT TYPE='button' name='smb' value='บันทึก' onclick='goto_url(1)'>&nbsp;&nbsp;<input type='button' value='ยกเลิก' onClick='history.go(-1);return true;'></td>";
echo "</tr>";
echo "</Table>";
echo "</form>";
}
...
switch($book_type) {
    case 1:
        ...
        while($row_answer = $result_answer->fetch_assoc()) {
            ...
            smartpush($person_id,"book"," เลขที่ ".$_POST[book_no]." : ".$_POST[subject],"มีหนังสือเข้า");
        }
...
//ส่วนของการแยกหน้า
	if($_SESSION['login_group']==1){
			if($_REQUEST['search_index']==1){	
					...
					$sql="select * from book_main where book_type='1' and  $_REQUEST[field] like '%$_REQUEST[search]%' and office='$_REQUEST[sub_department]' order by ms_id ";
					}
					else{
					$sql="select * from book_main where book_type='1' and $_REQUEST[field] like '%$_REQUEST[search]%' order by ms_id ";
					}
			}
			else{
			$sql="select * from book_main where book_type='1' and office='$department' order by ms_id ";
			}
	$dbquery = mysqli_query($connect,$sql);
	$num_rows = mysqli_num_rows($dbquery);
			...
			if($_REQUEST['search_index']==1){
			      if($_REQUEST['system_user_khet']!=""){
			       ...
			       $sql="select * from book_main where book_type='2' and office='$khet_code' and  $_REQUEST[field] like '%$_REQUEST[search]%' order by ms_id ";
				  }else{
			$sql="select * from book_main where book_type='2' and office='$_SESSION[system_user_khet]' order by ms_id ";
			}
			}
			else{
			$sql="select * from book_main where book_type='2' and office='$_SESSION[system_user_khet]' order by ms_id ";
...
<?php
	...
	$dbquery = mysqli_query($connect,$sql);
			...
			$sql="select * from book_main where book_type='2' and office='$_SESSION[system_user_khet]' and $_REQUEST[field] like '%$_REQUEST[search]%' order by ms_id DESC  limit $start,$pagelen";
			}
			else{
			$sql="select * from book_main where book_type='2' and office='$_SESSION[system_user_khet]' order by ms_id  DESC limit $start,$pagelen";
			}
			}
	$dbquery = mysqli_query($connect,$sql);
	}
...
'bookdetail','width=500,height=500,scrollbars')" title="คลิกเพื่อดูรายละเอียด"><span style="text-decoration: none"><FONT size="2" >คลิก</font></span></A></td>
...
echo "<tr><td colspan='8'>&nbsp;&nbsp;<FONT COLOR='#009933'><IMG SRC='modules/book/images/file1.gif' WIDTH='16' HEIGHT='16' BORDER='0'><FONT size='2' >มีไฟล์เอกสาร</font></td></tr>";
echo "</table>";
