...
//หาหน่วยงานและชื่อผู้ส่ง
$sql_sender = mysqli_query($connect,
"SELECT * FROM  system_workgroup  WHERE  workgroup='$row2[office]' ") ;
$row_sender= mysqli_fetch_array($sql_sender) ;
if($row_sender){
$sender=$row_sender[workgroup_desc];
		//หาชื่แผู้ส่ง	
		$sql_name = mysqli_query($connect,
"SELECT * FROM  person_main WHERE person_id='$row2[sender]'");
		$row_name= mysqli_fetch_array($sql_name) ;
}
else {
$sql_sender = mysqli_query($connect,
"SELECT * FROM  system_school WHERE  school_code='$row2[office]' ") ;
$row_sender= mysqli_fetch_array($sql_sender) ;
$sender=$row_sender['school_name'];
		//หาชื่แผู้ส่ง	
		$sql_name = mysqli_query($connect,
"SELECT * FROM  person_sch_main WHERE person_id='$row2[sender]'");
		$row_name= mysqli_fetch_array($sql_name) ;
}
...
if ($ans_num>0) {
$day_now=date("Y-m-d H:i:s");
				$sql_check_bookregister = mysqli_query($connect,
"SELECT * FROM  bookregister_year WHERE year_active='1' and  start_receive_num!='0' and school_code is null ") ;
				...
				$sql = "insert into bookregister_receive(year, register_number, book_no, signdate, book_from, book_to, subject, register_date, ref_id, officer, book_link, secret, workgroup) values ('$result_start[year]', '$register_number', '$book_no', '$row2[signdate]', '$sender', 'สำนักงานเขตพื้นที่การศึกษา', '$subject', '$day_now', '$ref_id', '$user', '$book_link', '$secret','$result_permission[p2]')";
...
$sql_work_group = mysqli_query($connect,
"SELECT * FROM  system_workgroup") ;
...
"SELECT * FROM person_main where  status='0' and department='$result_permission[p2]'") ;
...
"SELECT * FROM book_sendto_answer WHERE ref_id='$ref_id' and (status='2' or status='4') ");
