...
if(isset($_POST['return'])){
		if (isset($_POST['s_id'])) 
			{ 
				...
				mysqli_query($connect,"INSERT INTO book_sendto_answer (send_to, ref_id, school, status, forward_from, rec_forward_date) Values('$_POST[s_id]','$_POST[ref_id]', '$_POST[office_from]','6','$_POST[forward_name]','$day_now') ") ;
			}
}
...
// person answer
$sql_answer = mysqli_query($connect,"SELECT id FROM  book_sendto_answer WHERE ref_id ='$ref_id' and send_to='$user' and status='5' and answer='1' ") ;
...
$sql_answer = mysqli_query($connect,"update book_sendto_answer set answer='1', 
		answer_time='$day_now'
		where ref_id='$ref_id' and send_to='$user' and status='5' ") ;
}
...
$sql_name = "select * from book_sendto_answer where ref_id='$ref_id' and school='$_SESSION[user_school]' and send_to='$user' order by id";
...
<table border="0" width="100%" style="border-collapse: collapse">
		<tr>
			<td bgcolor="#003399"><font face="Tahoma"><font size="2">&nbsp;</font><span lang="th"><font size="2" color="#FFFFFF"><B>ส่งหนังสือคืนให้สารบรรณโรงเรียน</B></font></span></font> </td>
		</tr>
		</table>

  <form method="POST" action="bookdetail_school_person.php" name="form1" onSubmit="return checkform();">
	...
	<table border="0" width="400"  style="border-collapse: collapse">
		<form method="POST" action="" name="form2" >
			<tr>
				...
				จำนวน <FONT SIZE="2"><?php echo $num2;?></FONT> ราย</font></b></td>
			</tr>
			<tr>
				<td>
<?php
...
while ($r2=mysqli_fetch_array($result2)) {
	$send_to = $r2['send_to'] ;
?>&nbsp;<FONT SIZE="2">&nbsp;<FONT SIZE="2"><A HREF="bookdetail_school_person.php?index=2&sd_index=<?php echo $r2['id'];?>&b_id=<?php echo $_REQUEST['b_id'];?> "><IMG SRC="../../../images/b_drop.png" WIDTH="16" HEIGHT="16" BORDER="0" ALT="ลบออก"></A>&nbsp; <?php echo $list2;?>. <?php echo $office_name_ar[$send_to]; ?>&nbsp; 
