...
echo "<tr>";
echo "<td align='right'><span lang='th'>เลขที่หนังสือ&nbsp;</span></td><td colspan='2'>&nbsp;<input type='hidden' name='book_no' size='15'  value='$result_ref[book_no]'    style='background-color: #99ccff' required>$result_ref[book_no]</td></tr><tr>";
...
echo "<tr>";
echo "<td width='120' align='right' height='47'><span lang='th'>เนื้อหาโดยสรุป&nbsp;</span></td>";
...
echo "<tr>";
echo "<td align='center' colspan='4'> <button type='submit' class='btn btn-primary'><span class='glyphicon glyphicon-ok' name='sbm'></span>&nbsp;บันทึก</button>&nbsp;<button type='submit' class='btn btn-info' onClick='goto_url(0);return true;'><span class='glyphicon glyphicon-triangle-left'></span>&nbsp;กลับไปทะเบียนหนังสือส่ง</button></td>";
...
$sql = "insert into book_main (book_type, office, sender, level, secret, book_no, signdate, subject, detail, ref_id, send_date, bookregis_link) values ('$book_type', $_POST[department], '$user', '$_POST[level]', '$_POST[secret]', '$_POST[book_no]', '$_POST[signdate]','$_POST[subject]','$_POST[detail]','$_POST[ref_id]','$day_now', '1')";
$dbquery = mysqli_query($connect,$sql);
