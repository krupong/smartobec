...
<?php
	if($_SESSION['login_group']==1){
			if($_REQUEST['search_index']==1){	
					if($_REQUEST['sub_department']!=""){
					$sql="select * from book_main where book_type='1'  and $_REQUEST[field] like '%$_REQUEST[search]%' and office='$_REQUEST[sub_department]' order by ms_id  limit $start,$pagelen";
					}
					else{
			...
			$sql="select * from book_main where book_type='1' and office='$department' order by ms_id  limit $start,$pagelen";
			}
	$dbquery = mysqli_query($connect,$sql);
	}
	else if($_SESSION['login_group']>=2){
			...
			$sql="select * from book_main where book_type='2' and office='$_SESSION[system_user_khet]' and $_REQUEST[field] like '%$_REQUEST[search]%' order by ms_id   limit $start,$pagelen";
			}
			else{
			$sql="select * from book_main where book_type='2' and office='$_SESSION[system_user_khet]' order by ms_id  limit $start,$pagelen";
			}
			}
	$dbquery = mysqli_query($connect,$sql);
	}
...
echo "<tr><td colspan='8'>&nbsp;&nbsp;<FONT size='2' COLOR='#009933'><IMG SRC='modules/book/images/file1.gif' WIDTH='16' HEIGHT='16' BORDER='0'>มีไฟล์เอกสาร</td></tr>";
echo "</table>";
