...
echo "<td colspan='3' align='left'>&nbsp;&nbsp;<input type='radio' value='area_all' name='sendto' required>&nbsp;สพท.ทุกแห่ง";
echo "<br>&nbsp;&nbsp;<input type='radio' value='area_some' name='sendto' required onClick=\"window.open('modules/book/main/select_send.php?sd_index=some','PopUp','width=700,height=600,scrollbars,status'); \">&nbsp;สพท.บางแห่ง";
...
echo "<br>&nbsp;&nbsp;<input type='radio' value='dep_all' name='sendto' required onClick=\"window.open('modules/book/main/select_send_5.php?sd_index=all','PopUp','width=700,height=600,scrollbars,status'); \">&nbsp;สำนักใน สพฐ.";
...
//echo "$result_school[khet_precis]";
//echo "$_REQUEST[user_khet]";
...
echo "<br>&nbsp;&nbsp;<input type='radio' value='area_all' name='sendto' required>&nbsp;สพท.ทุกแห่ง";
echo "<br>&nbsp;&nbsp;<input type='radio' value='area_some' name='sendto' required onClick=\"window.open('modules/book/main/select_send.php?sd_index=some','PopUp','width=700,height=600,scrollbars,status'); \">&nbsp;สพท.บางแห่ง";
//echo "<br>&nbsp;&nbsp;<input type='radio' value='some' name='sendto' required onClick=\"window.open('modules/book/main/select_send_3.php?sd_index=some','PopUp','width=700,height=600,scrollbars,status'); \">&nbsp;หน่วยงาน/รร.สังกัด สำนักบริหารงานการศึกษาพิเศษ";
...
echo "<tr>";
echo "<td align='right'><span lang='th'>เลขที่หนังสือ&nbsp;</span></td><td colspan='2'>&nbsp;<input type='text' name='book_no' size='15' value='$ref_result[office_no]'  style='background-color: #99ccff' required></td></tr><tr>";
...
if ($send_num==0 and $_POST['sendto']!='area_all') {
...
// ***
if(($_SESSION['login_group']>=2) or ($_SESSION['login_status']>10) and ($_SESSION['login_status']<=14)){	
$sql_send_num = mysqli_query($connect,"SELECT * FROM book_sendto_answer WHERE ref_id='$_POST[ref_id]' ") ;
$send_num = mysqli_num_rows ($sql_send_num) ;
if ($send_num==0 and $_POST['sendto']=='area_some') {
echo "<div align='center'>";
echo "<B><FONT SIZE=2 COLOR=#990000>ยังไม่ได้ระบุผู้รับหนังสือ</B><BR><BR>" ;
 echo "&nbsp;&nbsp;&nbsp;<input type=\"button\" value=\"&nbsp;&nbsp;แก้ไข&nbsp;&nbsp;\" onClick=\"javascript:history.go(-1)\" ></CENTER>" ;
echo "</div>";
...
$day_now=date("Y-m-d H:i:s");
	 ...
	 if (($_SESSION['login_group']==1) and ($_POST['sendto']=='area_some')) {
	$book_type=1;  // สำนักถึงสพทบางแห่ง
}
	else if (($_SESSION['login_group']==1) and ($_POST['sendto']=='area_all')) {
	$book_type=1;  // สพท ถึง สพท
}
	else if (($_SESSION['login_group']==1) and ($_POST['sendto']=='dep_all')) {
	$book_type=4;  // สพท ถึง สพท
}

	else if($_SESSION['login_group']==1){
	$book_type=1;  // สพฐ.
	}

	else if (($_SESSION['login_group']>=2) and ($_POST['sendto']=='saraban')) {
	$book_type=2;  // สพท ถึง สพฐ
}
	else if (($_SESSION['login_group']>=2) and ($_POST['sendto']=='area_all')) {
	$book_type=3;  // สพท ถึง สพท
}

	else if (($_SESSION['login_group']>=2) and ($_POST['sendto']=='area_some')) {
	$book_type=3;  //สพท ถึง สพท
}
...
if($_SESSION['login_group']>=2){		
			...
			else if($_POST['sendto']!='area_some'){
...
//ส่วนของการแยกหน้า
	if (($_SESSION['login_group']==1) and ($_SESSION['login_status']<=5)){
			if($_REQUEST['search_index']==1){	
					if($_REQUEST['sub_department']!=""){
					$sql="select * from book_main where book_type='1' or book_type='4'  and  $_REQUEST[field] like '%$_REQUEST[search]%' and office='$_REQUEST[sub_department]' order by ms_id desc";
					}
					else{
					$sql="select * from book_main where book_type='1' or book_type='4'  and $_REQUEST[field] like '%$_REQUEST[search]%' order by ms_id desc";
					}
			}
			else{
			$sql="select * from book_main where book_type='1'  or book_type='4'  and office='$department' order by ms_id desc";
			}
	$dbquery = mysqli_query($connect,$sql);
	$num_rows = mysqli_num_rows($dbquery);
			...
			if($_REQUEST['search_index']==1){
			      if($khet_code!=""){
			       ...
			       $sql="select * from book_main where book_type='2' or book_type='3' and office='$khet_code' and  $_REQUEST[field] like '%$_REQUEST[search]%' order by ms_id desc";
				  }else{
			$sql="select * from book_main where book_type='2'  or book_type='3'  and office='$_SESSION[system_user_khet]' order by ms_id desc";
			}
			}
			else{
			$sql="select * from book_main where book_type='2' or book_type='3'  and office='$_SESSION[system_user_khet]' order by ms_id desc";
...
<?php
	if (($_SESSION['login_group']==1)or ($_SESSION['login_status']<=5)){
			if($_REQUEST['search_index']==1){	
					if($_REQUEST['sub_department']!=""){
					$sql="select * from book_main where book_type='1'   or book_type='4'  and $_REQUEST[field] like '%$_REQUEST[search]%' and office='$_REQUEST[sub_department]' order by ms_id desc limit $start,$pagelen";
					}
					else{
					$sql="select * from book_main where book_type='1'  or book_type='4'  and $_REQUEST[field] like '%$_REQUEST[search]%' order by ms_id desc limit $start,$pagelen";
					}
			}
			else{
			$sql="select * from book_main where book_type='1'  or book_type='4'  and office='$department' order by ms_id desc limit $start,$pagelen";
			}
	$dbquery = mysqli_query($connect,$sql);
	}
	else if( ($_SESSION['login_group']>=2) or ($_SESSION['login_status']>10) and ($_SESSION['login_status']<=15)){
			if($_REQUEST['search_index']==1){
					if($_REQUEST['khet_code']!=""){

			$sql="select * from book_main where book_type='2'  or book_type='3'  and office='$_REQUEST[khet_code]' and $_REQUEST[field] like '%$_REQUEST[search]%' order by ms_id desc  limit $start,$pagelen";
			}
			else{
			$sql="select * from book_main where book_type='2' or book_type='3'  and office='$_REQUEST[khet_code]' order by ms_id desc limit $start,$pagelen";
			}
			}
	$dbquery = mysqli_query($connect,$sql);
	}
...
else if($result['bookregis_link']==1 and $result['book_type']==3){
$file = mysqli_query($connect,"SELECT * FROM  bookregister_send_filebook WHERE ref_id='$ref_id' ") ;
}
else if($result['bookregis_link']==1 and $result['book_type']==4){
$file = mysqli_query($connect,"SELECT * FROM  bookregister_send_filebook_sch WHERE ref_id='$ref_id' ") ;
}
