...
echo "<br>&nbsp;&nbsp;<input type='radio' value='dep_some' name='sendto' required onClick=\"window.open('modules/book/main/select_send_5.php?sd_index=all','PopUp','width=700,height=600,scrollbars,status'); \">&nbsp;สำนักใน สพฐ.";
...
echo "<tr>";
echo "<td align='right'><span lang='th'>เลขที่หนังสือ&nbsp;</span></td><td colspan='2'>&nbsp;<input type='text' name='book_no' size='15' value='ที่'  style='background-color: #99ccff' required></td></tr><tr>";
...
$day_now=date("Y-m-d H:i:s");
	...
	if(($_SESSION['login_group']==1) and ($_POST['sendto']=='area_all')){
	$book_type=1;  //ผู้ส่งคือสพท
	}
	else if(($_SESSION['login_group']==1) and ($_POST['sendto']=='area_some')){
	$book_type=1;  //ผู้ส่งคือสพท
	}
	else if(($_SESSION['login_group']==1) and ($_POST['sendto']=='some')){
	$book_type=1;  //ผู้ส่งคือสพท
	}

	else if(($_SESSION['login_group']==1) and ($_POST['sendto']=='dep_some')){
	$book_type=4;  //ผู้ส่งคือสพท
	}
...
<?php
	if(($_SESSION['login_group']==1)or ($_SESSION['login_status']<=5)){
			if($_REQUEST['search_index']==1){	
					if($_REQUEST['sub_department']!=""){
					$sql="select * from book_main where book_type='1'  and $_REQUEST[field] like '%$_REQUEST[search]%' and office='$_REQUEST[sub_department]' order by ms_id desc limit $start,$pagelen";
					}
					else{
			...
			$sql="select * from book_main where book_type='1' and office='$department' order by ms_id desc limit $start,$pagelen";
			}
	$dbquery = mysqli_query($connect,$sql);
	}
			...
			$sql="select * from book_main where book_type='2' and office='$_REQUEST[khet_code]' and $_REQUEST[field] like '%$_REQUEST[search]%' order by ms_id desc  limit $start,$pagelen";
			}
			else{
			$sql="select * from book_main where book_type='2' and office='$_REQUEST[khet_code]' order by ms_id desc limit $start,$pagelen";
			}
			}
	$dbquery = mysqli_query($connect,$sql);
	}
