...
$(function(){
	$("select#khet_code").change(function(){
		var datalist2 = $.ajax({	// รับค่าจาก ajax เก็บไว้ที่ตัวแปร datalist2
			  url: "modules/book/return_permission_sch_khet_person.php", // ไฟล์สำหรับการกำหนดเงื่อนไข
...
<?php
$khet_code1 = $result_permission['p4'];  //รหัสเขต

$sql = "select * from book_permission where p4='$khet_code1'";
$dbquery = mysqli_query($connect,$sql);
//$ref_result = mysqli_fetch_array($dbquery);
$num_rows = mysqli_num_rows($dbquery);		//จำนวนสารบรรณที่กำหนด
...
$sql = "select * from  system_khet  where khet_code=$khet_code1";
$dbquery = mysqli_query($connect,$sql);
...
$sql = "select  * from person_khet_main where khet_code=$khet_code1 order by position_code,name";
$dbquery = mysqli_query($connect,$sql);
while($result = mysqli_fetch_array($dbquery)){
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=book&task=permission_sch_khet_person&index=3&id=$_GET[id]&page=$_REQUEST[page]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=book&task=permission_sch_khet_person&page=$_REQUEST[page]\"'";
echo "</td></tr></table>";
}
...
echo "<Select  name='khet_code'  id='khet_code' size='1'>";
echo  "<option  value = ''>- - - - เลือก - - - -</option>" ;
$sql = "select * from  system_khet  where khet_code=$khet_code1 order by khet_type,khet_code";
$dbquery = mysqli_query($connect,$sql);
While ($khet_result = mysqli_fetch_array($dbquery)){
...
//echo $num_rows1;
$number_person = "10"; //กำหนดจำนวนสารบรรณ สพท. 
if($num_rows1  < $number_person){ 
echo "<br><div class='col-md-12' align='left'><p><INPUT TYPE='button' class='btn btn-primary' name='smb' value='เพิ่มเจ้าหน้าที่' onclick='location.href=\"?option=book&task=permission_sch_khet_person&index=1\"'></p></div>";
	}
...
//echo "<table class='table table-bordered' width='100%' style='background-color:rgba(255,255,255,0.9)'>";
//echo "<Tr><Td colspan='6' align='left'><INPUT TYPE='button' class='btn btn-primary' name='smb' value='เพิ่มเจ้าหน้าที่' onclick='location.href=\"?option=book&task=permission_sch_khet_person&index=1\"'</Td></Tr>";
...
While ($result = mysqli_fetch_array($dbquery))
	{
		...
		//if($person_level==2){
		echo "<Td align='center' width='50' ><a href=?option=book&task=permission_sch_khet_person&index=2&id=$id&page=$page><span class='glyphicon glyphicon-trash' aria-hidden='true'></span></a></Td>
		<Td align='center' width='50'><a href=?option=book&task=permission_sch_khet_person&index=5&id=$id&page=$page><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a></Td>";
	//	}
	//	else{
	//	echo "<td></td><td></td>";
	//	}
...
echo "</tbody></Table>";
echo "<center>** กำหนดสารบรรณ สพท.ได้สูงสุด จำนวน <b>$number_person</b> คน **</center>";
echo "</td></tr></table>";
...
function goto_url(val){
	if(val==0){
		callfrm("?option=book&task=permission_sch_khet_person");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.khet_code.value == ""){
			...
			callfrm("?option=book&task=permission_sch_khet_person&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.khet_code.value == ""){
			...
			callfrm("?option=book&task=permission_sch_khet_person&index=6");   //page ประมวลผล
		}
	}
}
</script>
