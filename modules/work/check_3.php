...
//หน่วยงานผู้บริหาร
$managerdepart=0;
//หน่วนงานผู้ดูแลการลงเวลาผู้บริหาร : 4=สำนักอำนวยการ
$addmanagerdepart=4;
...
//ตรวจสอบการเข้าถึง
if($department!=$addmanagerdepart)  {
exit();
}
...
<INPUT TYPE="hidden" name=task value="check_3">
...
$rec_date=date("Y-m-d H:i:s");
    ...
    $dbquery_personid->bind_param("i", $managerdepart);
...
$postwork_opt="0.0";
      ...
      if($postperson_id==1 || $postperson_id==6 ){$postwork_opt="0.0";}   
       ...
       //echo "555 ".$postperson_id;
			if(($postperson_id>0) and ($postdelete!=1)){
                    ...
                    $dbquery_update->bind_param("ssssss", $postperson_id,$postwork_opt,$rec_date,$officer,$f2_date,$person_id);
                    ...
                    $dbquery_insert->bind_param("ssssss", $f2_date,$person_id,$postperson_id,$postwork_opt,$rec_date,$officer);
    ...
    echo "<script>document.location.href='?option=work&task=check_3&datepicker=$f2_date'; </script>\n";
...
$sql_person = "select * from person_main where status='0'and department = ?";
            ...
            $dbquery_person->bind_param("i",$managerdepart);
...
$sql= "select position_code,person_id from person_main where person_id=? ";
    ...
    if($result_position_id>0 and ($result_position_id<=11) ){
      $showwhereposit=" and ((position_code>0) and (position_code<=11)) ";
    }else{$showwhereposit="";}


echo "<form id='frm1' name='frm1'>";
$sql_show = "select * from person_main where status='0' and department = ? $showwhereposit order by department,position_code,person_order";
            $dbquery_show = $connect->prepare($sql_show);
            $dbquery_show->bind_param("i",$managerdepart);
...
function goto_url(val){
	if(val==0){
		callfrm("?option=work&task=check_3");   // page ย้อนกลับ
	}else if(val==1){
	   callfrm("?option=work&task=check_3");   //page ประมวลผล
	}
}
</script>
