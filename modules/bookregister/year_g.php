...
//นำเข้าข้อมูลบุคลากร
require_once("main/person_chk.php");	
//เรียกชื่อสำนัก
	$sql_d="select * from system_subdepartment where sub_department = '$sub_department_id' ";		 //รหัสกลุ่ม 
	$dbquery_d = mysqli_query($connect,$sql_d);
	$result_d = mysqli_fetch_array($dbquery_d);
	$sub_department_name = $result_d['sub_department_name'];
...
//echo "<table>";
//echo "<tr align='center'><td><font color='#006666' size='3'><strong>กำหนดปีปฏิทิน ($sub_department_name)</strong></font></td></tr>";
//echo "</table>";
}
echo "<div align='center'><font color='#006666' size='3'><strong>กำหนดปีปฏิทิน ($sub_department_name)</strong></font></center>";
//ส่วนฟอร์มรับข้อมูล
if($index==1){
echo "<Center>";
...
echo "<Tr><Td align='right'>กลุ่ม&nbsp;&nbsp;&nbsp;&nbsp;</Td><Td align='left'><Input Type='hidden' Name='sub_department' id='sub_department' Size='5' value='$sub_department_id'>$sub_department_id</Td></Tr>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=bookregister&task=year_g&index=3&id=$_GET[id]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=bookregister&task=year_g\"'";
echo "</td></tr></table>";
}
...
if($index==4){
	...
	$sql = "update bookregister_year set  year_active='0' where sub_department = $_POST[sub_department]";
	$dbquery = mysqli_query($connect,$sql);
	}
$rec_date = date("Y-m-d");
$sql = "insert into bookregister_year (year, year_active, start_receive_num, start_send_num, start_command_num, start_cer_num, school_code, department, sub_department) values ('$_POST[year]', '$_POST[year_active]', '$_POST[start_receive_num]', '$_POST[start_send_num]', '$_POST[start_command_num]', '$_POST[start_cer_num]', '$_POST[school_code]', '$_POST[department]', '$_POST[sub_department]')";
$dbquery = mysqli_query($connect,$sql);
}
...
echo "<Tr><Td align='right'>กลุ่ม&nbsp;&nbsp;&nbsp;&nbsp;</Td><Td align='left'><Input Type='hidden' Name='sub_department' id='sub_department' Size='5' value='$ref_result[sub_department]'>$ref_result[sub_department]</Td></Tr>";
...
$sql = "select * from  bookregister_year where sub_department = '$sub_department_id'  order by year ";
//$sql = "select * from  bookregister_year where school_code is null order by year ";
$dbquery = mysqli_query($connect,$sql);
//echo  "<table width='70%' border='0' align='center'>";
echo "<table class='table table-bordered' width='100%' style='background-color:rgba(255,255,255,0.9)'>";
echo "<Tr><Td colspan='8' align='left'><INPUT TYPE='button' name='smb' value='เพิ่มปีปฏิทิน' onclick='location.href=\"?option=bookregister&task=year_g&index=1\"'</Td></Tr>";
...
While ($result = mysqli_fetch_array($dbquery))
	{
		...
		echo "<Tr bgcolor=$color><Td align='center' width='50'>$N</Td><Td  align='center'>$year</Td><Td align='center'><a href=?option=bookregister&task=year_g&index=7&id=$id&year_active=$year_active>$active_pic</a></Td><td align='center'>$start_receive_num</td><td align='center'>$start_send_num</td><td align='center'>$start_command_num</td>
		<Td align='center'><a href=?option=bookregister&task=year_g&index=2&id=$id><img src=images/drop.png border='0' alt='ลบ'></a></a></Td>
		<Td align='center'><a href=?option=bookregister&task=year_g&index=5&id=$id><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>
	</Tr>";
$M++;
...
function goto_url(val){
	if(val==0){
		callfrm("?option=bookregister&task=year_g");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.year.value == ""){
			alert("กรุณากรอกปี");
			...
			callfrm("?option=bookregister&task=year_g&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.year.value == ""){
			...
			callfrm("?option=bookregister&task=year_g&index=6");   //page ประมวลผล
		}
	}
}
</script>
