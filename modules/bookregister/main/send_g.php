...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ทะเบียนหนังสือส่ง ($sub_department_name $department_name)</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>ลงทะเบียนหนังสือส่ง ($sub_department_name  $department_name)</Font>";
echo "</Cener>";
echo "<Br>";
...
$sql_person= "select  * from person_main  where sub_department=$sub_department order by id DESC";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=bookregister&task=main/send_g&index=3&id=$_GET[id]&page=$_REQUEST[page]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=bookregister&task=main/send_g&page=$_REQUEST[page]\"'";
echo "</td></tr></table>";
}
...
$sql = "select * from  bookregister_send_filebook where  ref_id='$result_ref[ref_id]' order by id DESC";
$dbquery = mysqli_query($connect,$sql);
...
echo  "<option  value = ''>เลือก</option>" ;
$sql_person = "select  * from person_main  where sub_department ='$sub_department_id' order by sub_department";
$dbquery_person = mysqli_query($connect,$sql_person);
While ($result_person= mysqli_fetch_array($dbquery_person))
   {
		$person_id = $result_person['person_id'];
...
if($_REQUEST['search_index']==1){
		...
		$sql="select * from bookregister_send where sub_department='$sub_department_id'  and $_REQUEST[field] like '%$_REQUEST[search]%' ";
		}
}
else{
$sql="select * from bookregister_send where sub_department='$sub_department_id' "; 
}
$dbquery = mysqli_query($connect,$sql);
$num_rows = mysqli_num_rows($dbquery );

$pagelen=20;  // 1_กำหนดแถวต่อหน้า
$url_link="option=bookregister&task=main/send_g&search_index=$_REQUEST[search_index]&field=$_REQUEST[field]&search=$_REQUEST[search]&department=$_REQUEST[department]";  // 2_กำหนดลิงค์ฺ
...
echo "<INPUT TYPE='button' name='smb' value='ลงทะเบียนหนังสือ' onclick='location.href=\"?option=bookregister&task=main/send_g&index=1\"'>";
echo "</td>";

?>
	<form method="POST" action="?option=bookregister&task=main/send_g">
<td align="right">
...
<table class="table table-striped table-bordered table-hover table-condensed">
				<tr>
					...
					<td align="center">
					<strong>วันลงทะเบียน</strong></font></td>
					<td align="center">
...
$sql="select * from bookregister_send where  sub_department='$sub_department_id' order by year desc,register_number_g DESC limit $start,$pagelen "; 
...
onclick="window.open('modules/bookregister/main/bookdetail_send.php?id=<?php echo $result['ms_id'];?>', 'bookdetail','width=500,height=500,scrollbars')" title="คลิกเพื่อดูรายละเอียด"><span class='glyphicon glyphicon-list-alt' aria-hidden='true'></span></A></td>
<?php
//ตั้งค่าเวลาให้ลบได้					
$now=time();
...
//สิทธิ์การตั้งเรื่องของการกำหนดแก้ไข
//if(($result['officer'])==$user and ($delete==1)){
if(($result['officer']==$user or $total_p2==1 or  $total_p3==1) and ($delete==1)){	
echo "<Td align='center'><a href=?option=bookregister&task=main/send_g&index=2&id=$id&page=$_REQUEST[page]><span class='glyphicon glyphicon-trash' aria-hidden='true'></span></a></Td>";
}
else{
echo "<td></td>";
}
//if(($result['officer'])==$user and ($delete==1)){
if(($result['officer']==$user or $total_p2=='1' or  $total_p3=='1') and ($delete==1)){
echo "<Td align='center'><a href=?option=bookregister&task=main/send_g&index=5&id=$id&page=$_REQUEST[page]><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a></Td>";
}
else{
echo "<td></td>";
}
...
if(($check_book_num==0) and ($result['officer']==$user or $total_p2=='1' or  $total_p3=='1')){
...
function goto_url(val){
	if(val==0){
		callfrm("?option=bookregister&task=main/send_g"); 
	}else if(val==1){
		...
		callfrm("?option=bookregister&task=main/send_g&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url2(val){
	...
	var vdfile5 = document.frm1.dfile5.value;
		...
		callfrm("?option=bookregister&task=main/send_g&index=6");   //page ประมวลผล
		}
	}
}
