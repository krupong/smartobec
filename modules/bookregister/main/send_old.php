...
if($_REQUEST['search_index']==1){
		if($_REQUEST['department']!=""){
		$sql="select * from bookregister_send where $_REQUEST[field] like '%$_REQUEST[search]%' and department='$_REQUEST[department]' order by year,register_number DESC limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_send where $_REQUEST[field] like '%$_REQUEST[search]%' order by year,register_number DESC limit $start,$pagelen "; 
		}
}
else{
$sql="select * from bookregister_send order by year,register_number DESC limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
