...
$result_start=mysqli_fetch_array($query_start);
		...
		//ปิดการทำงานที่มีอยู่เดิม (ปีเก่า) //////////////////////////
			$sql8 = "update bookregister_year set  year_active='0' where school_code is null";
		...
		$sql9 = "insert into bookregister_year (year, year_active, start_receive_num, start_send_num, start_command_num, start_cer_num, school_code, department, sub_department) values ('$this_year1', '1', '1', '1', '1', '1', '', '', '')";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=bookregister&task=main/send&index=3&id=$_GET[id]&page=$_REQUEST[page]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=bookregister&task=main/send&page=$_REQUEST[page]\"'";
echo "</td></tr></table>";
}
...
$sql = "update bookregister_send set book_no='$_POST[book_no]', signdate='$_POST[signdate]', book_from='$_POST[book_from]', book_to='$_POST[book_to]', subject='$_POST[subject]', department='$_POST[group]', operation='$_POST[operation]', comment='$_POST[comment]', secret='$_POST[secret]'  where ms_id='$_POST[id]' ";
$dbquery = mysqli_query($connect,$sql);
...
$url_link="option=bookregister&task=main/send&search_index=$_REQUEST[search_index]&field=$_REQUEST[field]&search=$_REQUEST[search]&department=$_REQUEST[department]";  // 2_กำหนดลิงค์ฺ
...
?>
	<form method="POST" action="?option=bookregister&task=main/send">
<td align="right">
...
<table class="table table-striped table-bordered table-hover table-condensed">
				<tr>
					<td align="center">
					<strong>เลขทะเบียนส่ง</strong></td>
					<td align="center">
					<strong>ปี</strong></td>
					<td align="center">
...
if($_REQUEST['search_index']==1){
		if($_REQUEST['department']!=""){
		$sql="select * from bookregister_send where $_REQUEST[field] like '%$_REQUEST[search]%' and department='$_REQUEST[department]' order by year desc, register_number DESC limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_send where $_REQUEST[field] like '%$_REQUEST[search]%' order by year desc, register_number DESC limit $start,$pagelen "; 
		}
}
else{
$sql="select * from bookregister_send order by year desc, register_number DESC limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
...
function goto_url(val){
	if(val==0){
		callfrm("?option=bookregister&task=main/send"); 
	}else if(val==1){
	...
	var vdfile5 = document.frm1.dfile5.value;
		...
		callfrm("?option=bookregister&task=main/send&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url2(val){
	...
	var vdfile5 = document.frm1.dfile5.value;
		...
		callfrm("?option=bookregister&task=main/send&index=6");   //page ประมวลผล
		}
	}
}
