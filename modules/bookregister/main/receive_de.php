...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ทะเบียนหนังสือรับ ($department_name)</strong></font></td></tr>";
echo "</table>";
}
...
$result_start=mysqli_fetch_array($query_start);
		if(!($result_start)){
			//echo "<div align='center'>ยังไม่ได้กำหนดปีปฏิทินการทำงาน สำนัก กรุณาแจ้งเจ้าหน้าที่ทะเบียน</div>";
			//exit();
		}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>ลงทะเบียนหนังสือรับ  ($department_name)</Font>";
echo "</Cener>";
echo "<Br>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=bookregister&task=main/receive_de&index=3&id=$_GET[id]&page=$_REQUEST[page]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=bookregister&task=main/receive_de&page=$_REQUEST[page]\"'";
echo "</td></tr></table>";
}
...
$ref_id = $_POST[ref_id];
			...
			//
			$sql_sendto=	"insert into book_sendto_answer (send_level, ref_id, send_to, answer, answer_time) values ('2', '$ref_id','$department_id2','1','$day_now')";
			$dbquery = mysqli_query($connect,$sql_sendto);
			//
			$sql_sendto2=	"insert into book_sendto_answer (send_level, ref_id, send_to, school, status, answer, answer_time) values ('2', '$ref_id','$sub_department_id2','$department_id2','1','NULL','$day_now')";
...
echo "<td colspan='3' align='left'>&nbsp;$result_ref[register_number_de]<input type='hidden' name='register_number' size='5' value='$result_ref[register_number]' ><input type='hidden' name='register_number_de' size='5' value='$result_ref[register_number_de]' ><input type='hidden' name='register_number_g' size='5' value='$result_ref[register_number_g]' ></td>";  //เลขรับสำนัก

if($result_ref[register_number_de]==''){  //ถ้า register_number_de เท่ากับ 0 ให้แสดงค่า regis_new เพื่อส่งค่าไปยัง answer
	$regis_new='0';
}
else {
	$regis_new='1';
...
//เลขทะเบียน  สำนัก หาเลขสุดท้าย
$sql_start_de="select * from bookregister_year where year_active='1' and department = '$department_id' ";
//$sql_start="select * from bookregister_year where year_active='1' and school_code is null";
$query_start_de=mysqli_query($connect,$sql_start_de);
$result_start_de=mysqli_fetch_array($query_start_de);

$sql_number_de="select  max(register_number_de) as number_max from bookregister_receive where year='$result_start_de[year]' and department = '$department_id' ";
$query_number_de=mysqli_query($connect,$sql_number_de);
$result_number_de=mysqli_fetch_array($query_number_de);

$register_number_de=$_POST['register_number_de'];

if($register_number_de > '0'){				//ถ้าเลขมากกว่า 0 ให้ใช้เลขเดิม
$register_number_de=$register_number_de;
}
else{
$register_number_de=$result_number_de['number_max']+1;  //ถ้าเลขเท่ากับ 0 ให้บวก 1 จากเลขเดิม
//$regis_new=$_POST['regis_new'];
//if($regis_new > '0'){				//ถ้าเลข == 0 ให้บันทึกลง  book_sendto_answer 
//}
//else{
//บันทึกในระบบรับส่งหนังสือราชการ
$day_now=date("Y-m-d H:i:s");
...
$ref_id = $_POST[ref_id];
			...
			//$sql_book = "insert into book_main (book_type, office, sender, level, secret, book_no, signdate, subject, detail, ref_id, send_date, bookregis_link) values ('', '$user', '$_SESSION[login_user_id]', '$level', '0', '$book_no', '$signdate','$subject','$detail','$ref_id','$day_now','2')";
			//$dbquery = mysqli_query($connect,$sql_book);

			//สำนักลงรับ
			$sql_sendto=	"insert into book_sendto_answer (send_level, ref_id, send_to, answer, answer_time) values ('', '$ref_id','$department_id2','1','$day_now')";
			$dbquery = mysqli_query($connect,$sql_sendto);

			//สำนักส่งต่อไปกลุ่ม
			$sql_sendto2=	"insert into book_sendto_answer (send_level, ref_id, send_to, school, status, answer, answer_time) values ('','$ref_id','$sub_department_id2','$department_id2','1','','$day_now')";
			$dbquery2 = mysqli_query($connect,$sql_sendto2);
		//	}   //if isset
//}
}

$sql = "update bookregister_receive set register_number_de=$register_number_de, book_no='$_POST[book_no]', signdate='$_POST[signdate]', book_from='$_POST[book_from]', book_to='$_POST[book_to]', subject='$_POST[subject]', sub_department='$_POST[group]', operation='$_POST[operation]', comment='$_POST[comment]', level='$_POST[level]'  where ms_id='$_POST[id]' ";
$dbquery = mysqli_query($connect,$sql);
//echo $sql;
...
if($_REQUEST['search_index']==1){
		if($_REQUEST['department']!=""){
		$sql="select * from bookregister_receive where  department='$department_id' and $_REQUEST[field] like '%$_REQUEST[search]%' ";
...
$url_link="option=bookregister&task=main/receive_de&search_index=$_REQUEST[search_index]&field=$_REQUEST[field]&search=$_REQUEST[search]&department=$department_id";  // 2_กำหนดลิงค์ฺ
...
echo "<INPUT TYPE='button' name='smb' value='ลงทะเบียนหนังสือ' onclick='location.href=\"?option=bookregister&task=main/receive_de&index=1\"'>";
echo "</td>";

?>
	<form name="frm1" method="POST" action="?option=bookregister&task=main/receive_de">
<td align="center">
...
<table class='table table-bordered' width='100%' style='background-color:rgba(255,255,255,0.9)'>
				<tr bgcolor="#FFCCCC">
					...
					<font size="2" face="Tahoma">เลขทะเบียนรับสำนัก</font></td>
					<td align="center" width="50">
...
if($_REQUEST['search_index']==1){
		if($_REQUEST['department']!=""){
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%' and department='$department_id' order by year DESC,register_number_de  DESC  limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%'  and department='$department_id' order by year DESC,register_number_de  DESC  limit $start,$pagelen "; 
		}
		...
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%' and sub_department='$sub_department' order by year DESC,register_number_de DESC  limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%'  and department='$department_id' order by year DESC,register_number_de  DESC limit $start,$pagelen "; 
		}
...
$sql="select * from bookregister_receive where  department='$department_id' order by year DESC,register_number_de DESC limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
...
//if($register_number_de==0){
//	$register_number_de="<font color='#FF0000'><a href=?option=book&task=main/receive&saraban_index=2&search_index=&field=&search=&page=1><img src=images/add.png border='0' alt='ยังไม่ลงรับหนังสือ'><br>ไปหน้าลงรับ</a></font>";

	//ตรวจสอบว่าลงรับหนังสือหรือยังไม่ลงรับ
if($register_number_de==0){
	$register_number_de="<font color='#FF0000'><a href=?option=bookregister&task=main/receive_de_2&index=5&id=$id&page=$_REQUEST[page]&department=$department_id><img src=images/add.png border='0' alt='ยังไม่ลงรับหนังสือ'><br>ลงรับ</a></font>";
}
else{
	$register_number_de=$register_number_de;
}
...
?>
			<tr bgcolor="<?php echo $color;?>">
					...
					<td align="left"><?php echo $subject;?>&nbsp;<?php echo $file_img;?>&nbsp;<?php if($register_number_de==0){ ?><font color='#FF0000'>(ยังไม่ลงรับหนังสือ)</font><?php } ?></td>
					<td align="left"><?php 
...
if(($result['officer']==$user) and ($delete==1)){
echo "<Td align='center'><a href=?option=bookregister&task=main/receive_de&index=5&id=$id&page=$_REQUEST[page]&department=$_REQUEST[department]><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>";
}
else{
echo "<td></td>";
}
...
echo "<tr><td colspan='5'>&nbsp;&nbsp;<FONT COLOR='#009933'><IMG SRC='modules/bookregister/images/file1.gif' WIDTH='16' HEIGHT='16' BORDER='0'>มีไฟล์เอกสาร</FONT>&nbsp;&nbsp;&nbsp;<a href='modules/bookregister/main/export_receive_all.php?department=$department_id' target='_blank'>ส่งออกทั้งหมด Excel ปีปัจจุบัน</a>";
?><form id='form1' name='form1' method='post' action='modules/bookregister/main/export_receive_all_day.php?department=<?php echo $department_id ?>' target='_blank'><td colspan='1'>เลือกวันที่ </td><td colspan='4'><input type='submit' name='button' id='button' value='ส่งออก Excel' /><script>DateInput('date_value', true, 'YYYY-MM-DD')</script><script>DateInput('date_value2', true, 'YYYY-MM-DD')</script></td></form>
<?php
...
function goto_url(val){
	if(val==0){
		callfrm("?option=bookregister&task=main/receive_de"); 
	}else if(val==1){
	...
	var vdfile5 = document.frm1.dfile5.value;
		...
		callfrm("?option=bookregister&task=main/receive_de&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url2(val){
	...
	var vdfile5 = document.frm1.dfile5.value;
		...
		callfrm("?option=bookregister&task=main/receive_de&index=6");   //page ประมวลผล
		}
	}
}
