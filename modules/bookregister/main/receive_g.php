...
//echo "</table>";
echo "<div align='center'><font color='#006666' size='3'><strong>ทะเบียนหนังสือรับ ($sub_department_name $department_name)</strong></font></center>";
}
...
echo "<td colspan='3' align='left'>&nbsp;<input type='text' class='textinput' name='book_from' size='80'  style='background-color: #E7D8EB'></td>";
echo "</tr>";

//echo "<tr>";
//echo "<td align='right'><span lang='th'><font size='2' color='#0000FF'>เรียน&nbsp;</font></span></td>";
...
echo "<td align='left'>";
echo "<div id='book_to' align='left'><Select  name='book_to' class='textinput' id='book_to'  size='1' style='background-color: #FFDDFF'>";
...
echo "<td colspan='3' align='left'>&nbsp;<input type='text' name='subject' class='textinput' size='80'  style='background-color: #E7D8EB'></td>";
echo "</tr>";

echo "<tr>";
echo "<td align='right'><span lang='th'><font size='2' color='#0000FF'>บุคคลปฏิบัติ&nbsp;</font></span></td>";
echo "<td colspan='3' align='left'>&nbsp;<Select  name='operation' class='textinput' id='operation'  size='1' style='background-color: #FFDDFF'>";
echo  "&nbsp;&nbsp;<option  value = ''>เลือก</option>" ;
$sql_person= "select  * from person_main  where sub_department='$sub_department_id'";
...
echo "<td colspan='3' align='left'>&nbsp;<input type='text' class='textinput' name='comment' size='50'  style='background-color: #FFDDFF' value='เอกสารกระดาษ'></td>";
echo "</tr>";
...
echo "<td width='274' align='left'>&nbsp;<input type='file' class='textinput' name='myfile1' size='26' style='background-color: #FFDDFF'></td>";
...
echo "<td width='274' align='left'>&nbsp;<input type='file' class='textinput' name='myfile2' size='26' style='background-color: #FFDDFF'> </td>";
...
echo "<td width='274' align='left'>&nbsp;<input type='file' class='textinput' name='myfile3' size='26' style='background-color: #FFDDFF'> </td>";
...
echo "</tr>";
echo "<tr>";
echo "<td align='center' colspan='4'><BR><INPUT TYPE='button' class='btmew' name='smb' value='ตกลง' onclick='goto_url(1)' class=entrybutton>&nbsp;&nbsp;<input type='reset' value='Reset' name='reset'></td>";
echo "</tr>";
echo "</Table>";
echo "</form>";
...
$signdate = $_POST[signdate];
			...
			//
			$sql_sendto=	"insert into book_sendto_answer (send_level, ref_id, send_to, answer, answer_time) values ('2', '$ref_id','$sub_department_id2','1','$day_now')";
			$dbquery = mysqli_query($connect,$sql_sendto);
			//
			$sql_sendto2=	"insert into book_sendto_answer (send_level, ref_id, send_to, school, status, answer, answer_time) values ('2', '$ref_id','$operation','$sub_department_id2','1','NULL','')";
...
//ถ้าเลข == 0 ให้บันทึกลง  book_sendto_answer 
//บันทึกในระบบรับส่งหนังสือราชการ
$day_now=date("Y-m-d H:i:s");
$user = $_SESSION[login_user_id];
$level = $_POST[level];
...
$book_no = $_POST[book_no];
$department_id2 = $_POST[department];
$sub_department_id2 = $_POST[group];
$signdate = $_POST[signdate];
$ref_id = $_POST[ref_id];
$operation=$_POST[operation];

			//กลุ่มรับ
			$sql_sendto=	"insert into book_sendto_answer (send_level, ref_id, send_to, answer, answer_time) values ('', '$ref_id','$sub_department_id2','1','$day_now')";
			$dbquery = mysqli_query($connect,$sql_sendto);
			
			//กลุ่มส่งต่อไปบุคคล
			$sql_sendto2=	"insert into book_sendto_answer (send_level, ref_id, send_to, school, status, answer, answer_time) values ('', '$ref_id','$operation','$sub_department_id2','1','','$day_now')";
			$dbquery = mysqli_query($connect,$sql_sendto2);
...
<table class="table table-striped table-bordered table-hover table-condensed">
				<tr>
					<td align="center">
					<strong>เลขทะเบียนรับกลุ่ม</strong></td>
					<td align="center">
					<strong>ปี</strong></td>
					<td align="center">
...
if($_REQUEST['search_index']==1){
		if($_REQUEST['sub_department']!=""){
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%' and sub_department='$sub_department_id' order by year DESC,register_number  DESC limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%'  and sub_department='$sub_department_id' order by year DESC,register_number  DESC  limit $start,$pagelen "; 
		}
		...
		if($_REQUEST['sub_department']!=""){
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%' and sub_department='$sub_department' order by year DESC,register_number_g  DESC  limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%'  and sub_department='$sub_department_id' order by year DESC,register_number_g DESC   limit $start,$pagelen "; 
		}

		//ถ้าไม่มีค่าอะไรให้แสดงทั้งหมด
}
else{
$sql="select * from bookregister_receive where  sub_department='$sub_department_id' order by year DESC,register_number_g DESC  limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
...
//	$register_number_g="<font color='#FF0000'><a href=?option=book&task=main/receive&saraban_index=3&search_index=&field=&search=&page=1><img src=images/add.png border='0' alt='ยังไม่ลงรับหนังสือ'><br>ลงรับ</a></font>";
...
echo "<Td align='center'><a href=?option=bookregister&task=main/receive_g&index=2&id=$id&page=$_REQUEST[page]><span class='glyphicon glyphicon-remove' aria-hidden='true'></span></a></Td>";
}
else{
echo "<td></td>";
}
if(($result['officer']==$user) and ($delete==1)){
echo "<Td align='center'><a href=?option=bookregister&task=main/receive_g&index=5&id=$id&page=$_REQUEST[page]&sub_department=$_REQUEST[sub_department]><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a></Td>";
}
else{
echo "<td></td>";
}
...
//echo "<tr><td colspan='14'>&nbsp;&nbsp;<FONT COLOR='#009933'><IMG SRC='modules/bookregister/images/file1.gif' WIDTH='16' HEIGHT='16' BORDER='0'>มีไฟล์เอกสาร</FONT>&nbsp;&nbsp;&nbsp;<a href='modules/bookregister/main/export_receive_all.php ' target='_blank'>ส่งออกExcelปีปัจจุบัน</a>";
echo "<tr><td colspan='5'>&nbsp;&nbsp;<FONT COLOR='#009933'><IMG SRC='modules/bookregister/images/file1.gif' WIDTH='16' HEIGHT='16' BORDER='0'>มีไฟล์เอกสาร</FONT>&nbsp;&nbsp;&nbsp;<a href='modules/bookregister/main/export_receive_all.php?sub_department=$sub_department_id' target='_blank'>ส่งออกทั้งหมด Excel ปีปัจจุบัน</a>";
?><form id='form1' name='form1' method='post' action='modules/bookregister/main/export_receive_all_day.php?sub_department=<?php echo $sub_department_id ?>' target='_blank'><td colspan='1'>เลือกวันที่ </td><td colspan='4'><input type='submit' name='button' id='button' value='ส่งออก Excel' /><script>DateInput('date_value', true, 'YYYY-MM-DD')</script><script>DateInput('date_value2', true, 'YYYY-MM-DD')</script></td></form>
<?php
