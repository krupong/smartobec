...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ทะเบียนคำสั่ง สพฐ.</strong></font></td></tr>";
echo "</table>";
}
...
echo "<tr>";
echo "<td width='94' align='right' height='50'><font size='2' color='#0000FF'>ไฟล์แนบ&nbsp;</font></td>";
...
$sql = "insert into bookregister_command(year, register_number, book_no, signdate, subject, comment, register_date, officer,file_name, file_des, department) values ('$result_start[year]', '$register_number', '$book_number', '$_POST[signdate]', '$_POST[subject]',  '$_POST[comment]', '$day_now', '$user', '$myfile1name', '$_POST[file_des]', '$department')";
$dbquery = mysqli_query($connect,$sql);
...
echo "<input type='hidden' name='department' size='50'  style='background-color: #FFDDFF' value='$result_ref[department]'>";
//echo "</tr>";
...
$sql = "update bookregister_command set  signdate='$_POST[signdate]', subject='$_POST[subject]', comment='$_POST[comment]',officer='$user', department='$_POST[department]' where ms_id='$_POST[id]' ";
$dbquery = mysqli_query($connect,$sql);
...
<?php
if($_REQUEST['search_index']==1){
$sql="select *,bookregister_command.officer,bookregister_command.department from bookregister_command left join person_main on bookregister_command.officer=person_main.person_id where  $_REQUEST[field] like '%$_REQUEST[search]%' order by year,register_number  limit $start,$pagelen "; 
}
else{
$sql="select *,bookregister_command.officer,bookregister_command.department from bookregister_command left join person_main on bookregister_command.officer=person_main.person_id order by year,register_number limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
...
//เรียกชื่อสำนัก
	...
	$result_d = mysqli_fetch_array($dbquery_d);
	$department_name = $result_d['department_name'];
	$department_precis = $result_d['department_precis'];
...
?>
			<tr bgcolor="<?php echo $color;?>">
					...
					<td align='left'><?php echo $result['prename'].$result['name']." ".$result['surname'];?> (<?php echo $department_precis; ?>)</td>
...
$timestamp_recdate_2=$timestamp_recdate+(86400*15);  //เพิ่มเวลา 15 วัน
...
echo "<tr><td colspan='5' align='left'><b>หมายเหตุ</b>&nbsp;ปรับปรุงทะเบียนได้ภายใน 15 วัน หลังลงทะเบียน</td><td colspan='6' align='left'>";
echo "ส่งออกปีปัจจุบัน[Excel]<a href='modules/bookregister/main/export_to_excel3.php?year_index=$result_start[year]' target='_blank'>[คลิก]</a>";
echo "</td></tr>";
