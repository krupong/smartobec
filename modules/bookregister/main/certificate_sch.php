...
$sql = "select * from  bookregister_certificate_sch where school_code='$_SESSION[user_school]' and officer='$_SESSION[login_user_id]' order by ms_id desc limit 0,1";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=bookregister&task=main/certificate_sch&index=3&id=$_GET[id]&page=$_REQUEST[page]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=bookregister&task=main/certificate_sch&page=$_REQUEST[page]\"'";
echo "</td></tr></table>";
}

//ส่วนลบข้อมูล
if($index==3){
$sql = "delete from bookregister_certificate_sch where ms_id='$_GET[id]' ";
$dbquery = mysqli_query($connect,$sql);
}
...
$sql_number="select  max(register_number) as number_max from bookregister_certificate_sch where school_code='$_SESSION[user_school]' and year='$result_start[year]' ";
...
$sql = "insert into bookregister_certificate_sch(school_code, year, register_number, book_no, signdate, name_cer, subject, comment, register_date, officer,file_name) values ('$_SESSION[user_school]', '$result_start[year]', '$register_number', '$book_number', '$_POST[signdate]', '$_POST[name_cer]', '$_POST[subject]', '$_POST[comment]', '$day_now', '$user', '$myfile1name')";
$dbquery = mysqli_query($connect,$sql);
...
echo "</Cener>";
echo "<Br><Br>";
$sql = "select * from  bookregister_certificate_sch where ms_id='$_GET[id]'";
$dbquery = mysqli_query($connect,$sql);
$result_ref = mysqli_fetch_array($dbquery);
...
$sql = "select file_name from  bookregister_certificate_sch where ms_id='$_POST[id]'";
$dbquery = mysqli_query($connect,$sql);
$result_ref = mysqli_fetch_array($dbquery);
...
$sql = "update bookregister_certificate_sch set  name_cer='$_POST[name_cer]', signdate='$_POST[signdate]', subject='$_POST[subject]', comment='$_POST[comment]' where ms_id='$_POST[id]' ";
$dbquery = mysqli_query($connect,$sql);
...
$myfile1name=$ref_id.".".$lastname1 ; 
copy ($myfile1, "modules/bookregister/upload_files4/".$myfile1name)  ; 
$sql = "update bookregister_certificate_sch set file_name='$myfile1name' where ms_id='$_POST[id]' "; 
$dbquery = mysqli_query($connect,$sql);
unlink ($myfile1) ;}
} //end if
...
$sql="select * from bookregister_certificate_sch where $_REQUEST[field] like '%$_REQUEST[search]%' and school_code='$_SESSION[user_school]' ";
}
else{
$sql="select * from bookregister_certificate_sch where school_code='$_SESSION[user_school]' ";  
}
$dbquery = mysqli_query($connect,$sql);
$num_rows = mysqli_num_rows($dbquery );

$pagelen=20;  // 1_กำหนดแถวต่อหน้า
$url_link="option=bookregister&task=main/certificate_sch&search_index=$_REQUEST[search_index]&field=$_REQUEST[field]&search=$_REQUEST[search]";  // 2_กำหนดลิงค์ฺ
...
echo "<INPUT TYPE='button' name='smb' value='ลงทะเบียนเกียรติบัตร' onclick='location.href=\"?option=bookregister&task=main/certificate_sch&index=1\"'>";
echo "</td>";

?>
	<form method="POST" action="?option=bookregister&task=main/certificate_sch">
<td align="right">
...
<?php
if($_REQUEST['search_index']==1){
$sql="select *,bookregister_certificate_sch.officer from bookregister_certificate_sch left join person_sch_main on bookregister_certificate_sch.officer=person_sch_main.person_id where bookregister_certificate_sch.school_code='$_SESSION[user_school]' and $_REQUEST[field] like '%$_REQUEST[search]%' order by year,register_number  limit $start,$pagelen "; 
}
else{
$sql="select *,bookregister_certificate_sch.officer from bookregister_certificate_sch left join person_sch_main on bookregister_certificate_sch.officer=person_sch_main.person_id  where bookregister_certificate_sch.school_code='$_SESSION[user_school]' order by year,register_number limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
...
echo "<Td align='center'><a href=?option=bookregister&task=main/certificate_sch&index=2&id=$id&page=$_REQUEST[page]><img src=images/drop.png border='0' alt='ลบ'></a></Td>";
}
else{
echo "<td></td>";
}
if(($result['officer'])==$user and ($delete==1)){
echo "<Td align='center'><a href=?option=bookregister&task=main/certificate_sch&index=5&id=$id&page=$_REQUEST[page]><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>";
}
else{
echo "<td></td>";
}
...
echo "<tr><td colspan='12'><a href='modules/bookregister/export_certificate_sch.php?year_index=$result_start[year]&school=$_SESSION[user_school]' target='_blank'>ส่งออกExcel</a></td></tr>";
echo "</table>";
}  //end index
...
function goto_url(val){
	if(val==0){
		callfrm("?option=bookregister&task=main/certificate_sch"); 
	}else if(val==1){
	var file1 = document.frm1.myfile1.value;
		...
		callfrm("?option=bookregister&task=main/certificate_sch&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url2(val){
	...
	var file1 = document.frm1.myfile1.value;
		...
		callfrm("?option=bookregister&task=main/certificate_sch&index=6");   //page ประมวลผล
		}
	}
}
