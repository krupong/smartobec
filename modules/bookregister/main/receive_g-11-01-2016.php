...
if($_REQUEST['search_index']==1){
		if($_REQUEST['sub_department']!=""){
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%' and sub_department='$sub_department_id' order by year,register_number  DESC limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%'  and sub_department='$sub_department_id' order by year,register_number  DESC  limit $start,$pagelen "; 
		}
		...
		if($_REQUEST['sub_department']!=""){
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%' and sub_department='$sub_department' order by year,register_number_g  DESC  limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%'  and sub_department='$sub_department_id' order by year,register_number_g DESC   limit $start,$pagelen "; 
		}

		//ถ้าไม่มีค่าอะไรให้แสดงทั้งหมด
}
else{
$sql="select * from bookregister_receive where  sub_department='$sub_department_id' order by year,register_number_g DESC  limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
