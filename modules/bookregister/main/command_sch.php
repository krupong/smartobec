...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ทะเบียนคำสั่ง $department_name</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>ลงทะเบียนคำสั่ง $department_name</Font>";
echo "</Cener>";
echo "<Br>";
...
echo "</tr>";
echo "<tr>";
echo "<td align='right' height='50'><span lang='th'><font size='2' color='#0000FF'>เรื่อง&nbsp;</font></span></td>";
echo "<td colspan='3' align='left'>&nbsp;&nbsp;<input type='text' name='subject' size='80'  style='background-color: #E7D8EB'></td>";
echo "</tr>";
echo "<tr>";
echo "<td align='right' height='50'><span lang='th'><font size='2' color='#0000FF'>สั่ง ณ วันที่&nbsp;</font></span></td>";

echo "<td colspan='3' align='left'  height='50'>";
?>
...
echo "</tr>";
echo "<tr>";
echo "<td align='right' height='50'><span lang='th'><font size='2' color='#0000FF'>หมายเหตุ&nbsp;</font></span></td>";
echo "<td colspan='3' align='left'>&nbsp;<input type='text' name='comment' size='50'  style='background-color: #FFDDFF'></td>";
echo "</tr>";

echo "<tr>";
echo "<td width='371' align='right' colspan='3' height='50'><p align='center'><font size='2' color='#800000'>แนบไฟล์(ถ้ามี)</font></td>";
echo "</tr>";
...
echo "<tr>";
echo "<td align='center' colspan='4' height='50'><FONT SIZE='2' COLOR='#CC9900'>เฉพาะไฟล์ doc, docx, pdf, xls, xlsx, gif, jpg, zip, rar เท่านั้น</FONT></td>";
echo "</tr>";
echo "<tr>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=bookregister&task=main/command_sch&index=3&id=$_GET[id]&page=$_REQUEST[page]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=bookregister&task=main/command_sch&page=$_REQUEST[page]\"'";
echo "</td></tr></table>";
}

//ส่วนลบข้อมูล
if($index==3){
$sql = "delete from bookregister_command_sch where ms_id='$_GET[id]' ";
...
$sql_number="select  max(register_number) as number_max from bookregister_command_sch where department='$department' and year='$result_start[year]' ";
...
$sql = "insert into bookregister_command_sch(department, year, register_number, book_no, signdate, subject, comment, register_date, officer,file_name) values ('$department', '$result_start[year]', '$register_number', '$book_number', '$_POST[signdate]', '$_POST[subject]',  '$_POST[comment]', '$day_now', '$user', '$myfile1name')";
$dbquery = mysqli_query($connect,$sql);
...
echo "</Cener>";
echo "<Br><Br>";
$sql = "select * from  bookregister_command_sch where ms_id='$_GET[id]'";
$dbquery = mysqli_query($connect,$sql);
...
echo "<tr>";
echo "<td width='94' align='right' height='50'><font size='2' color='#0000FF'>ไฟล์แนบ 1&nbsp;</font></td>";
...
$sql = "select file_name from  bookregister_command_sch where ms_id='$_POST[id]'";
$dbquery = mysqli_query($connect,$sql);
$result_ref = mysqli_fetch_array($dbquery);
...
$sql = "update bookregister_command_sch set  signdate='$_POST[signdate]', subject='$_POST[subject]', comment='$_POST[comment]' where ms_id='$_POST[id]' ";
$dbquery = mysqli_query($connect,$sql);
...
$myfile1name=$ref_id.".".$lastname1 ; 
copy ($myfile1, "modules/bookregister/upload_files3/".$myfile1name)  ; 
$sql = "update bookregister_command_sch set file_name='$myfile1name' where ms_id='$_POST[id]' "; 
$dbquery = mysqli_query($connect,$sql);
unlink ($myfile1) ;}
} //end if
...
$sql="select * from bookregister_command_sch where $_REQUEST[field] like '%$_REQUEST[search]%' and department='$department' ";
}
else{
$sql="select * from bookregister_command_sch where department='$department' ";  
}
$dbquery = mysqli_query($connect,$sql);
$num_rows = mysqli_num_rows($dbquery );

$pagelen=20;  // 1_กำหนดแถวต่อหน้า
$url_link="option=bookregister&task=main/command_sch&search_index=$_REQUEST[search_index]&field=$_REQUEST[field]&search=$_REQUEST[search]";  // 2_กำหนดลิงค์ฺ
...
echo "<INPUT TYPE='button' name='smb' value='ลงทะเบียนคำสั่ง' onclick='location.href=\"?option=bookregister&task=main/command_sch&index=1\"'>";
echo "</td>";

?>
	<form method="POST" action="?option=bookregister&task=main/command_sch">
<td align="right">
...
<?php
if($_REQUEST['search_index']==1){
$sql="select *,bookregister_command_sch.officer from bookregister_command_sch left join person_main on bookregister_command_sch.officer=person_main.person_id where bookregister_command_sch.department='$department' and $_REQUEST[field] like '%$_REQUEST[search]%' order by year,register_number limit $start,$pagelen "; 
}
else{
$sql="select *,bookregister_command_sch.officer from bookregister_command_sch left join person_main on bookregister_command_sch.officer=person_main.person_id where bookregister_command_sch.department='$department' order by year,register_number limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
...
echo "<Td align='center'><a href=?option=bookregister&task=main/command_sch&index=2&id=$id&page=$_REQUEST[page]><img src=images/drop.png border='0' alt='ลบ'></a></Td>";
}
else{
echo "<td></td>";
}
if(($result['officer'])==$user and ($delete==1)){
echo "<Td align='center'><a href=?option=bookregister&task=main/command_sch&index=5&id=$id&page=$_REQUEST[page]><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>";
}
else{
echo "<td></td>";
}
...
echo "<tr><td colspan='11'><a href='modules/bookregister/export_command_sch.php?year_index=$result_start[year]&department=$department' target='_blank'>ส่งออกExcel</a></td></tr>";
echo "</table>";
}  //end index
...
function goto_url(val){
	if(val==0){
		callfrm("?option=bookregister&task=main/command_sch"); 
	}else if(val==1){
	var file1 = document.frm1.myfile1.value;
		...
		callfrm("?option=bookregister&task=main/command_sch&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url2(val){
	...
	var file1 = document.frm1.myfile1.value;
		...
		callfrm("?option=bookregister&task=main/command_sch&index=6");   //page ประมวลผล
		}
	}
}
