...
echo "<td colspan='3' align='left'>&nbsp;$result_ref[register_number_de]<input type='hidden' name='register_number_de' size='5' value='$register_number_de' ><input type='hidden' name='register_number_g' size='5' value='$result_ref[register_number_g]' ></td>";  //เลขรับสำนัก
echo "</tr>";
...
if($_REQUEST['search_index']==1){
		if($_REQUEST['department']!=""){
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%' and department='$department_id' order by year,register_number  limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%'  and department='$department_id' order by year,register_number  limit $start,$pagelen "; 
		}
		...
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%' and sub_department='$sub_department' order by year,register_number  limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%'  and department='$department_id' order by year,register_number  limit $start,$pagelen "; 
		}
...
$sql="select * from bookregister_receive where  department='$department_id' order by year,register_number limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
...
if($register_number_de==0){
	$register_number_de="<font color='#FF0000'><a href=?option=bookregister&task=main/receive_de&index=99&id=$id><img src=images/drop.png border='0' alt='ยังไม่ลงรับหนังสือ'></a></font>";
}
else{
	$register_number_de=$register_number_de;
}
