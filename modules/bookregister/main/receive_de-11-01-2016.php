...
if($_REQUEST['search_index']==1){
		if($_REQUEST['department']!=""){
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%' and department='$department_id' order by year,register_number_de  DESC  limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%'  and department='$department_id' order by year,register_number_de  DESC  limit $start,$pagelen "; 
		}
		...
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%' and sub_department='$sub_department' order by year,register_number_de DESC  limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_receive where $_REQUEST[field] like '%$_REQUEST[search]%'  and department='$department_id' order by year,register_number_de  DESC limit $start,$pagelen "; 
		}
...
$sql="select * from bookregister_receive where  department='$department_id' order by year,register_number_de DESC limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
