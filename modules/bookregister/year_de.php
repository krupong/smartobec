...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>กำหนดปีปฏิทิน ($department_name)</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Tr><Td align='right'>สำนัก&nbsp;&nbsp;&nbsp;&nbsp;</Td><Td align='left'><Input Type='hidden' Name='department' id='department' Size='5' value='$department_id'>$department_id</Td></Tr>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=bookregister&task=year_de&index=3&id=$_GET[id]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=bookregister&task=year_de\"'";
echo "</td></tr></table>";
}
...
if($index==4){
	...
	$sql = "update bookregister_year set  year_active='0' where department = $_POST[department]";
	$dbquery = mysqli_query($connect,$sql);
	}
$rec_date = date("Y-m-d");
...
echo "<Tr><Td align='right'>สำนัก&nbsp;&nbsp;&nbsp;&nbsp;</Td><Td align='left'><Input Type='hidden' Name='department' id='department' Size='5' value='$ref_result[department]'>$ref_result[department]</Td></Tr>";
...
$sql = "select * from  bookregister_year where department = '$department_id' and ((sub_department is null) or (sub_department='')) order by year ";
...
While ($result = mysqli_fetch_array($dbquery))
	{
		...
		echo "<Tr bgcolor=$color><Td align='center' width='50'>$N</Td><Td  align='center'>$year</Td><Td align='center'><a href=?option=bookregister&task=year_de&index=7&id=$id&year_active=$year_active>$active_pic</a></Td><td align='center'>$start_receive_num</td><td align='center'>$start_send_num</td><td align='center'>$start_command_num</td>
		<Td align='center'><a href=?option=bookregister&task=year_de&index=2&id=$id><img src=images/drop.png border='0' alt='ลบ'></a></a></Td>
		<Td align='center'><a href=?option=bookregister&task=year_de&index=5&id=$id><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>
	</Tr>";
$M++;
...
function goto_url(val){
	if(val==0){
		callfrm("?option=bookregister&task=year_de");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.year.value == ""){
			alert("กรุณากรอกปี");
			...
			callfrm("?option=bookregister&task=year_de&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.year.value == ""){
			...
			callfrm("?option=bookregister&task=year_de&index=6");   //page ประมวลผล
		}
	}
}
</script>
