...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>เจ้าหน้าที่ ($department_name)</strong></font></td></tr>";
echo "</table>";
}
...
$sql = "select  * from person_main where status='0' and department='$department_id' order by name";
...
//echo  "<option  value = ''>เลือก</option>" ;
$sql = "select  * from system_department  where department='$department_id'  order by department ASC";
$dbquery = mysqli_query($connect,$sql);
While ($result = mysqli_fetch_array($dbquery))
   {
		$department = $result['department'];
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=bookregister&task=permission_de&index=3&id=$_GET[id]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=bookregister&task=permission_de\"'";
echo "</td></tr></table>";
}
...
$dbquery = mysqli_query($connect,$sql);
echo "<script>document.location.href='?option=bookregister&task=permission_de'; </script>\n";
}
...
echo   "<tr><td align='right'>กำหนดบทบาทหน้าที่&nbsp;&nbsp;</td>";
echo   "<td align='left'><input  type=radio name='saraban_status' value='2' $s2_check1>สารบรรณสำนัก&nbsp;&nbsp;<input  type=radio name='saraban_status' value='3' $s3_check1>สารบรรณกลุ่ม</td></tr>";
...
echo "<Tr><Td colspan='8' align='left'><INPUT TYPE='button' class='btn btn-primary' name='smb' value='เพิ่มเจ้าหน้าที่' onclick='location.href=\"?option=bookregister&task=permission_de&index=1\"'</Td></Tr>";
...
While ($result = mysqli_fetch_array($dbquery))
	...
	$sub_department_name = $result_g['sub_department_name'];
		echo "<Tr bgcolor=$color><Td align='center' width='50'>$M</Td><Td  align='left'>$name $surname <br>$sub_department_name  $department_name</Td>
  
  <Td  align='center'>$s2_pic</Td>
  <Td  align='center'>$s3_pic</Td>
  <Td align='center'>$p1_pic</Td>
		<Td align='center' width='50' ><a href=?option=bookregister&task=permission_de&index=2&id=$id><img src=images/drop.png border='0' alt='ลบ'></a></Td>
		<Td align='center' width='50'><a href=?option=bookregister&task=permission_de&index=5&id=$id><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>
	</Tr>";
$M++;
	}
echo "</Table>";
...
function goto_url(val){
	if(val==0){
		callfrm("?option=bookregister&task=permission_de");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.person_id.value == ""){
			alert("กรุณาเลือกบุคลากร");
		}else{
			callfrm("?option=bookregister&task=permission_de&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.person_id.value == ""){
			...
			callfrm("?option=bookregister&task=permission_de&index=6");   //page ประมวลผล
		}
	}
}
</script>
