...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=bookregister&task=year_sch&index=3&id=$_GET[id]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=bookregister&task=year_sch\"'";
echo "</td></tr></table>";
}
...
if($index==4){
	...
	$sql = "update bookregister_year set  year_active='0'  where school_code='$_SESSION[user_school]' ";
	$dbquery = mysqli_query($connect,$sql);
	}
$rec_date = date("Y-m-d");
$sql = "insert into bookregister_year (year, year_active, start_receive_num, start_send_num, start_command_num, start_cer_num, school_code, officer, rec_date) values ('$_POST[year]', '$_POST[year_active]', '$_POST[start_receive_num]', '$_POST[start_send_num]', '$_POST[start_command_num]', '$_POST[start_cer_num]', '$_SESSION[user_school]', '$_SESSION[login_user_id]', '$rec_date')";
$dbquery = mysqli_query($connect,$sql);
}
...
$sql = "select * from  bookregister_year where school_code='$_SESSION[user_school]' order by year";
$dbquery = mysqli_query($connect,$sql);
echo  "<table width='70%' border='0' align='center'>";
echo "<Tr><Td colspan='5' align='left'><INPUT TYPE='button' name='smb' value='เพิ่มปีปฏิทิน' onclick='location.href=\"?option=bookregister&task=year_sch&index=1\"'</Td></Tr>";
...
While ($result = mysqli_fetch_array($dbquery))
	{
		...
		echo "<Tr bgcolor=$color><Td align='center' width='50'>$N</Td><Td  align='center'>$year</Td><Td align='center'><a href=?option=bookregister&task=year_sch&index=7&id=$id&year_active=$year_active>$active_pic</a></Td><td align='center'>$start_receive_num</td><td align='center'>$start_send_num</td><td align='center'>$start_command_num</td><td align='center'>$start_cer_num</td>
		<Td align='center'><a href=?option=bookregister&task=year_sch&index=2&id=$id><img src=images/drop.png border='0' alt='ลบ'></a></a></Td>
		<Td align='center'><a href=?option=bookregister&task=year_sch&index=5&id=$id><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>
	</Tr>";
$M++;
...
function goto_url(val){
	if(val==0){
		callfrm("?option=bookregister&task=year_sch");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.year.value == ""){
			alert("กรุณากรอกปี");
			...
			callfrm("?option=bookregister&task=year_sch&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.year.value == ""){
			...
			callfrm("?option=bookregister&task=year_sch&index=6");   //page ประมวลผล
		}
	}
}
</script>
