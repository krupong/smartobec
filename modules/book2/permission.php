...
if($index==4){
$rec_date = date("Y-m-d");
$sql = "insert into bookregister_permission (person_id, p1, officer,rec_date) values ('$_POST[person_id]', '$_POST[bookregister_permission1]','$_SESSION[login_user_id]','$rec_date')";
$dbquery = mysqli_query($connect,$sql);
echo "<script>document.location.href='?option=bookregister&task=permission'; </script>\n";
}
...
if ($index==6){
$rec_date = date("Y-m-d");
$sql = "update bookregister_permission set  person_id='$_POST[person_id]', p1='$_POST[bookregister_permission1]', officer='$_SESSION[login_user_id]', rec_date='$rec_date' where id='$_POST[id]'";
$dbquery = mysqli_query($connect,$sql);
echo "<script>document.location.href='?option=bookregister&task=permission'; </script>\n";
}
...
$sql = "select bookregister_permission.id, bookregister_permission.p1, person_main.name, person_main.surname from bookregister_permission left join person_main on bookregister_permission.person_id=person_main.person_id where bookregister_permission.school_code is null order by bookregister_permission.id";
$dbquery = mysqli_query($connect,$sql);
echo  "<table width=50% border=0 align=center>";
