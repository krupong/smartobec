...
echo "<td colspan='3' align='left'>&nbsp;<input type='text' name='book_from' size='80'  style='background-color: #E7D8EB' value='$_SESSION[office_name]'></td>";
echo "</tr>";
...
$sql = "insert into bookregister_send(year, register_number, book_no, signdate, book_from, book_to, subject, workgroup, operation, comment, register_date, ref_id, officer, secret) values ('$result_start[year]', '$register_number', '$book_number', '$_POST[signdate]', '$_POST[book_from]', '$_POST[book_to]', '$_POST[subject]', '$_POST[workgroup]', '$_POST[operation]', '$_POST[comment]', '$day_now', '$ref_id', '$user', '$_POST[secret]')";
$dbquery = mysqli_query($connect,$sql);
...
$sql = "update bookregister_send set book_no='$_POST[book_no]', signdate='$_POST[signdate]', book_from='$_POST[book_from]', book_to='$_POST[book_to]', subject='$_POST[subject]', workgroup='$_POST[group]', operation='$_POST[operation]', comment='$_POST[comment]', secret='$_POST[secret]'  where ms_id='$_POST[id]' ";
$dbquery = mysqli_query($connect,$sql);
...
if($_REQUEST['search_index']==1){
		...
		$sql="select * from bookregister_send where $_REQUEST[field] like '%$_REQUEST[search]%' and workgroup='$_REQUEST[workgroup]' ";
		}
		else{
...
$url_link="option=bookregister&task=main/send&search_index=$_REQUEST[search_index]&field=$_REQUEST[field]&search=$_REQUEST[search]&workgroup=$_REQUEST[workgroup]";  // 2_กำหนดลิงค์ฺ
...
<table border="1" width="99%" id="table2" style="border-collapse: collapse" align="center">
				<tr bgcolor="#99FFFF">
					...
					<td align="center" width="30">
					<font face="Tahoma" size="2">ส่ง<br />ร.ร.</font></td>
					<td align="center" width="30">
					<font face="Tahoma" size="2">ส่ง<br />สพฐ.</font></td>
				</tr>
<?php
if($_REQUEST['search_index']==1){
		if($_REQUEST['workgroup']!=""){
		$sql="select * from bookregister_send where $_REQUEST[field] like '%$_REQUEST[search]%' and workgroup='$_REQUEST[workgroup]' order by year,register_number limit $start,$pagelen "; 
		}
		else{
		$sql="select * from bookregister_send where $_REQUEST[field] like '%$_REQUEST[search]%' order by year,register_number limit $start,$pagelen "; 
		}
}
else{
...
?>
			<tr bgcolor="<?php echo $color;?>">
					...
					<td align="left" valign="top"><?php echo $subject;?>&nbsp;<?php echo $file_img;?>&nbsp;<?php echo $secret_txt;?></td>
					<td align="left"><?php 
...
if(($check_book_num==0) and ($result['officer']==$_SESSION['login_user_id']) and ($delete==1)){
...
if(($result['officer']==$_SESSION['login_user_id']) and ($delete==1)){
echo "<td align='center'><a href=?option=bookobec&task=main/send&from_register=1&id=$id><img src=images/next.gif border='0' alt='ส่งสพฐ.'></td>";
}
else{
echo "<td></td>";
}
