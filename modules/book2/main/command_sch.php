...
$sql_number="select  max(register_number) as number_max from bookregister_command_sch where school_code='$_SESSION[user_school]' and year='$result_start[year]' ";
...
$sql = "insert into bookregister_command_sch(school_code, year, register_number, book_no, signdate, subject, comment, register_date, officer,file_name) values ('$_SESSION[user_school]', '$result_start[year]', '$register_number', '$book_number', '$_POST[signdate]', '$_POST[subject]',  '$_POST[comment]', '$day_now', '$user', '$myfile1name')";
$dbquery = mysqli_query($connect,$sql);
...
$sql="select * from bookregister_command_sch where $_REQUEST[field] like '%$_REQUEST[search]%' and school_code='$_SESSION[user_school]' ";
}
else{
$sql="select * from bookregister_command_sch where school_code='$_SESSION[user_school]' ";  
}
$dbquery = mysqli_query($connect,$sql);
...
<?php
if($_REQUEST['search_index']==1){
$sql="select *,bookregister_command_sch.officer from bookregister_command_sch left join person_sch_main on bookregister_command_sch.officer=person_sch_main.person_id where bookregister_command_sch.school_code='$_SESSION[user_school]' and $_REQUEST[field] like '%$_REQUEST[search]%' order by year,register_number limit $start,$pagelen "; 
}
else{
$sql="select *,bookregister_command_sch.officer from bookregister_command_sch left join person_sch_main on bookregister_command_sch.officer=person_sch_main.person_id where bookregister_command_sch.school_code='$_SESSION[user_school]' order by year,register_number limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
...
echo "<tr><td colspan='11'><a href='modules/bookregister/export_command_sch.php?year_index=$result_start[year]&school=$_SESSION[user_school]' target='_blank'>ส่งออกExcel</a></td></tr>";
echo "</table>";
}  //end index
