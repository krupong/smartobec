...
echo "<Select  name='school_code' id='school_code' size='1' onchange='e_show()'>";
echo  "<option  value = ''>เลือก</option>" ;
echo  "<option  value = 'other'>********หน่วยงานอื่น ๆ********</option>" ;
...
echo "<tr>";
echo "<td align='right'><span lang='th'><font size='2' color='#0000FF'><div id='from1' style='display:none'>ระบุจาก&nbsp;<div></font></span></td>";
echo "<td colspan='3' align='left'>&nbsp;<div id='from2' style='display:none'><input type='text' name='book_from' id='book_from' size='80'  style='background-color: #E7D8EB'><div></td>";
echo "</tr><div>";

echo "<tr>";
echo "<td align='right'><span lang='th'><font size='2' color='#0000FF'>ถึง&nbsp;</font></span></td>";
echo "<td colspan='3' align='left'>&nbsp;<input type='text' name='book_to' size='80'  style='background-color: #E7D8EB' value='สำนักงานเขตพื้นที่การศึกษา'></td>";
echo "</tr>";
...
$array_last5 = explode("." ,$myfile5_name) ;
 ...
 if ($lastname5 =="doc" or $lastname5 =="docx" or $lastname5 =="rar" or $lastname5 =="pdf" or $lastname5 =="xls" or $lastname5 =="xlsx" or $lastname5 =="zip" or $lastname5 =="jpg" or $lastname5 =="gif") { 
  ...
  ####
////
		//2/2016  หาชื่อโรงเรียน 
		if($_POST['school_code']!='other' and $_POST['school_code']!=''){
		$sql = "select * from system_school where school_code='$_POST[school_code]'";
		$dbquery = mysqli_query($connect,$sql);
		$school_result = mysqli_fetch_array($dbquery);
		$_POST['book_from']=$school_result['school_name'];
		$office=$_POST['school_code'];
		}
		else{
		$office="other";
		}
...
unlink ($myfile5) ;
			...
			///ส่วนการบันทึกระบบรับส่งหนังสือราชการ
			$sql_book = "insert into book_main (book_type, office, sender, level, secret, bookno, signdate, subject, detail, ref_id, send_date, bookregis_link) values ('6', '$office', '$_SESSION[login_user_id]', '1', '0', '$_POST[book_no]', '$_POST[signdate]','$_POST[subject]','ส่งต่อมาจากการลงทะเบียนหนังสือราชการ','$ref_id','$day_now','6')";
			$dbquery = mysqli_query($connect,$sql_book);
			$sql_sendto=	"insert into book_sendto_answer (send_level, ref_id, send_to) values ('2', '$ref_id','$_POST[workgroup]')";
...
$timestamp_recdate_2=$timestamp_recdate+(86400*1);  //เพิ่มเวลา 1 วัน
...
if($_REQUEST['workgroup']!=""){
echo "ส่งออกกลุ่ม[Excel]เฉพาะที่ลงทะเบียนเอกสารกระดาษ<a href='modules/bookregister/main/export_to_excel.php?workgroup=$_REQUEST[workgroup]' target='_blank'>[คลิก]</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ส่งออกกลุ่ม[Excel]ทั้งหมด<a href='modules/bookregister/main/export_to_excel2.php?workgroup=$_REQUEST[workgroup]' target='_blank'>[คลิก]</a>";
}
echo "</td></tr>";
echo "</table>";
}  //end index
...
function goto_url(val){
	...
	var vdfile5 = document.frm1.dfile5.value;
          ...
          document.frm1.book_no.focus();    
		   ...
		   else if (document.frm1.book_from.value=="" && (document.frm1.school_code.value=="" || document.frm1.school_code.value=="other"))
           {
...
function e_show(){
		if(document.getElementById("school_code").value=="other"){
