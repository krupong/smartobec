...
//หาข้อมูลบุคลากร
    ...
    $query_person->bind_param("s", $user);
    $query_person->execute();
    $result_qperson=$query_person->get_result();
    While ($result_person = mysqli_fetch_array($result_qperson))
   {
        $login_person_id=$result_person['person_id']; 
        $login_prename=$result_person['prename']; 
        $login_name=$result_person['name'];  
        $login_surname=$result_person['surname'];  
        $login_department=$result_person['department'];  
   }   
        ...
        <section class="content">
              ...
              <div class="box  container">
                <div class="box-header">
                  <h3 class="box-title">รายการหนังสือลงทะเบียนรับแล้วรอดำเนินการ</h3>
...
//หาหนังสือรับใหม่
    $sql_booksystem="select * from book2_system where receiver_department=? and receiver_status=? order by  receiver_no DESC ";
    ...
    While ($result_booksystem = mysqli_fetch_array($result_qbooksystem))
   {
        ...
        $book2system_receiver_group=$result_booksystem['receiver_group']; 
        $book2system_receiver_department=$result_booksystem['receiver_department']; 
        //$book2system_receiver_id=$result_booksystem['receiver_id']; 
        $book2system_receiver_officer=$result_booksystem['receiver_officer']; 
        $book2system_receiver_role=$result_booksystem['receiver_role']; 
...
?>
                  ...
                  <div class="row text-center" style="padding-bottom: 5px;padding-top: 15px;">
                        <div class="form-group">
                            <button type="button"  id="forwardbuttongroup"  class="btn btn-success" style="margin: 8px;"  data-toggle="modal" data-target="#forwardsendgroup" disabled><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span> ส่งต่อแบบกลุ่ม</button>
