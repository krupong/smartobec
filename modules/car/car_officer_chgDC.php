...
if(!(($index==1)  or ($index==2) or ($index==5) or ($index==7))){
    $header = "แก้ไขคนขับรถและรถกรณีอนุมัติไปแล้ว";
...
if ($index==5){
    ...
    if($ed!=0){
        ...
        if($ed==1 || $subm=="car_group" || $subm=="car_commander"){
            ...
            $header="แก้ไขคนขับรถกรณีที่อนุมัติไปแล้ว";
        }
...
<?php } ?>	
      ...
      <?php 
        $disabled="";
        if($commander_grant==2) $disabled="disabled";
      ?>
      <div class="form-group">
      ...
      <div class="form-group">
          ...
          <div class="col-sm-2 input-group">
            <span class="input-group-addon">
                <input type="radio" aria-label="..." name='commander_grant' value='1' <?=$disabled?> <?=$cG1?> checked></input>
            </span>
...
?>
          ...
          <div class="col-sm-3 input-group">
            <span class="input-group-addon">
                <input type="radio" aria-label="..." name='commander_grant2' value='2' checked></input>
            </span>
            <input type="text" class="form-control" value="อนุมัติ" readonly>              
            <span class="input-group-addon">
...
while($result_ajax = mysqli_fetch_array($query_ajax)){
    ...
    $selected="";
    if($car==$car_code) $selected="selected";
    /*
    ...
    */
    $num_rows=0;
    if($num_rows==0){
        echo "<option value='$car_code' $selected>$car_type_name $name ทะเบียน $car_number</option>";
    }
}
                    ?>
                    </select></div>
      ...
      <div class="form-group">
          <label class="col-sm-3 control-label text-right"></label>
          <div class="col-sm-8 input-group"><span class="input-group-addon">ความคิดเห็น</span><Input Type='Text' Name='officer_comment2' class="form-control" value="<?=$officer_comment?>">
          </div>
        </div>
...
if ($index==6){
    $date_time_now = date("Y-m-d H:i:s");
            ...
            officer_comment='$_POST[officer_comment2]',
            car='$_POST[car]',
            officer_sign='$user',
...
if($action=""){
    ...
    if(isset($_POST['dep_search'])!=""){
        ...
        if($subm=="car_commander"){
            ...
            $sql = $sql. " and a.commander_grant=2 
            ...
            $sql = $sql. "and ( IFNULL(a.commander_grant,0)='2' or '$searchstatusid'='999')  order by a.commander_grant ,a.car_start,a.car_finish ";
      ...
      <div class="panel-body">
        <div class="row">
            <div class="text-center">
               <form class="form-inline" action="?option=car&task=car_officer_chgDC<?php echo "&subm=".$subm;?>" enctype="multipart/form-data" method="POST">
    <div class="form-group">
...
// split page
    ...
    $url_link="option=car&task=car_officer_chgDC&searchtext=$searchtext&searchstatusid=$searchstatusid";  
...
if( ($commander_grant==0 and $subm=="car_commander") or $commander_grant==1){
    ?>
    <Td><a href=?option=car&task=car_officer_chgDC&car_index=<?=$car_index?>&page=<?=$page?>&index=5&id=<?=$id?>&ed=2&subm=<?=$subm?> class='btn btn-warning'><span class='glyphicon glyphicon-pencil' ></span> อนุมัติเอกสาร</a></Td>
        <?php 
}else if($commander_grant==2){
?>
                <Td colspan=2>
                    <a href=?option=car&task=car_officer_chgDC&car_index=<?=$car_index?>&page=<?=$page?>&index=5&id=<?=$id?>&ed=2&subm=<?=$subm?> class='btn btn-warning'><span class='glyphicon glyphicon-scale' ></span> แก้ไข</a>
                </Td>
<?php }
else{
echo "<td></td>";
...
function goto_url_ed(ed,val){
	if(val==0){
        if(ed==3){
            ...
            callfrm("?option=car&task=car_officer_chgDC&subm=<?=$subm?>");   // page ย้อนกลับ
        }
	}else if(val==1){
        if("<?=$subm?>"=="car_commander"){
            if(frm1.commander_grant.value==""){
                ...
                callfrm("?option=car&task=car_officer_chgDC&index=6&subm=<?=$subm?>");
            }
