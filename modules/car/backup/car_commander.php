...
$date_time_now = date("Y-m-d H:i:s");
	foreach($_POST as $key => $value){
		$sql = "update car_main set commander_grant='$value', commander_sign='$_SESSION[login_user_id]', commander_date='$date_time_now' where id='$key'";
		$dbquery = mysqli_query($connect,$sql);
	}
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>การอนุมัติ</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
...
echo "<Tr align='left'><Td align='right'>ลงชื่อ&nbsp;&nbsp;</Td><Td>$full_name_ar[$group_sign]</Td></Tr>";
echo "<Tr align='left'><Td align='right'>วดป&nbsp;&nbsp;</Td><Td>";
...
echo "<Tr ><Td colspan='2' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=car&task=main/car_commander&page=$_GET[page]&car_index=$car_index\"'></Td></Tr>";
$sql = "select * from car_main where id='$_GET[id]'";
$dbquery = mysqli_query($connect,$sql);
$ref_result = mysqli_fetch_array($dbquery);
$id=$ref_result['id'];
...
$url_link="option=car&task=main/car_commander&car_index=$car_index";  // 2_กำหนดลิงค์ฺ
...
if($car_index>=1){
$sql="select car_main.id, car_main.person_id, car_main.car_start, car_main.car_finish, car_main.rec_date, car_main.officer_sign, car_main.group_sign, car_main.commander_sign, car_main.commander_grant, car_main.commander_date, car_car.name  from car_main left join car_car on  car_main.car=car_car.car_code  where car_main.car='$car_index' and car_main.officer_sign is not null order by car_start, car_car.car_code limit $start,$pagelen";
}
else{
$sql="select car_main.id, car_main.person_id, car_main.car_start, car_main.car_finish, car_main.rec_date,  car_main.officer_sign, car_main.group_sign, car_main.commander_sign, car_main.commander_grant, car_main.commander_date, car_car.name  from car_main left join car_car on  car_main.car=car_car.car_code  where car_main.officer_sign is not null order by car_start, car_car.car_code limit $start,$pagelen";
}
$dbquery = mysqli_query($connect,$sql);
...
echo "</Td>";
echo "</Tr>";
echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='60'>เลขที่</Td><Td width='120'>วันเริ่มใช้รถ</Td><Td width='120'>วันสิ้นสุดการใช้</Td><Td>รถ</Td><Td>ผู้ขอใช้</Td><Td width='120'>วดป ขออนุญาต</Td><Td  width='120'>อนุมัติ/คำสั่ง</Td><Td width='70'>รายละเอียด</Td><Td><INPUT TYPE='button' name='smb' value='อนุมัติ' onclick='goto_url(1)'></Td><Td width='50' >บันทึก</Td></Tr>";
...
function goto_url(val){
	if(val==0){
		callfrm("?option=car&task=main/car_commander");   // page ย้อนกลับ
	}else if(val==1){
		callfrm("?option=car&task=main/car_commander&index=4");   //page ประมวลผล
	}
}
function goto_url_update(val){
	if(val==0){
		...
		if(!(frm1.commander_grant[0].checked || frm1.commander_grant[1].checked)){
			...
			callfrm("?option=car&task=main/car_commander&index=6");   //page ประมวลผล
		}
	}
}

function goto_url2(val){
callfrm("?option=car&task=main/car_commander");
}
