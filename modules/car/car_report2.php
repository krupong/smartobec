...
$depname="";
if(isset($_GET['depname']))$depname=$_GET['depname'];
...
//ส่วนของการแยกหน้า
    ...
    $sql="select * from system_subdepartment 
        ...
        <div class="row">
            <div class="col-md-10 text-left">
                <ul class="nav nav-pills">
                    <li role="presentation" class=""><a href="?option=car&task=car_report1&y=<?=$y?>"><h3><?=$depname?></h3></a></li>
                </ul>
            </div>
    ...
    while($i<3){
        ...
        ?>
                  <li role="presentation" class="<?=$active?>"><a href="?option=car&task=car_report2&y=<?=$currentYear-$i?>&department=<?=$department?>&depname=<?=$depname?>"><?=$currentYear-$i?></a></li>
        <?php $i++;}?>
                </ul>
            </div>
...
?>
              <Tr>
                  <Td><?=$N?></Td>
                  <Td><?php echo $result['sub_department_name'];?></Td>
<?php
    $sql2=" select month(a.car_start) as mcs, count(month(a.car_start)) as cmc 
    from car_main a
    left outer join person_main b on a.person_id=b.person_id
    where b.department = $department
    and b.sub_department=$sub_department
