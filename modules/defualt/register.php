...
$rec_date = date("Y-m-d");		
$sqlusergroup = "select * from person_main where person_id = '$_SESSION[login_user_id]'";
$resultusergroup = mysqli_query($connect,$sqlusergroup);
$resultusergroupnum = mysqli_num_rows($resultusergroup);
if($resultusergroupnum!=0){
	$sql = "insert into system_user (person_id, username, userpass,status,school_user,officer,rec_date) values ( '$_SESSION[login_user_id]','$username','$passname','1','0','$_SESSION[login_user_id]','$rec_date')";
}else{
